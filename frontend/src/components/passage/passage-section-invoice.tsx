  ...
  return (
    <div className="border-2 border-black bg-white">
      ...
      <div className="px-4 pt-4 pb-2">
        <h2 className="text-center text-lg font-bold border-b-2 border-black pb-1 mb-0.5">
          {invoice.title}
        </h2>
          ...
          {invoice.items.map((row, rowIdx) => (
            <tr key={rowIdx} className="border-b border-black">
              {row.map((cell, cellIdx) => (
                <td key={cellIdx} className="px-2 py-1 border-r border-black last:border-r-0">
                  {cell}
                </td>
              ))}
            </tr>
        ...
        <div className="flex flex-col items-end">
          {invoice.summary.map((item, idx) => (
            <div key={idx} className="flex justify-between w-48">
              <span className={idx === invoice.summary.length - 1 ? 'font-bold' : ''}>
                {item.label}
              </span>
              <span className={idx === invoice.summary.length - 1 ? 'font-bold' : ''}>
                {item.value}
