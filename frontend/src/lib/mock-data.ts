...
（C） invite～to do「～に…するよう招く」。`,
  metacogFeedbackJa: `目的を問う設問では、出来事の説明よりも「書き手が最後に何をしたいのか（感謝・苦情・依頼・提案など）」を示す評価語と結論文を最優先で拾うのが最重要です。見分け方は、設問が Why did X write? / What is the purpose of the letter? のように「理由・目的」を直接聞いてくることです。途中にトラブル報告があっても、結論が称賛・感謝なら目的はそちらに寄ります。

①設問を先に見て、「目的＝感情＋行動（感謝する／抗議する／依頼する等）」のどれを探すか決める。
②本文では冒頭段落を「背景（何が起きたか）」として速く読み、トラブルや状況説明はメモ程度に留める。
③次に中盤〜末尾で、評価語（素晴らしい、感謝している、失望した等）と将来の行動宣言（また利用する、改善してほしい等）を探し、そこを目的の根拠として確定する。
④選択肢は「本文に出てくる単語がある」だけで選ばず、目的に直結する発話行為（称賛／懸念表明／招待／誤り報告）が本文の結論文と一致するかで切る（トラブル描写に引っ張られて苦情系を選びやすいので注意）。
⑤複数テキストなら、広告はサービス内容の前提確認にだけ使い、目的判断は手紙の結論部分を最優先に置く。

「何が起きたか」ではなく「だから何を伝えるために書いたのか」を末尾の評価・感謝・要求で決める。トラブルの記述は背景、目的は結論文に出る。`,
};
...
（C） be unable to do「〜することができない」。`,
  metacogFeedbackJa: `この設問タイプで最重要な読み方の結論は、「人物についての推測」は人物の発言だけで完結させず、別テキストの条件（料金・規定・サービス内容）と突き合わせて確定することです。設問が “What is suggested about …?” のように直接書かれていない情報を聞く形なら、本文間の情報連結で答えを作るタイプだと見分けます。今回は「選んだサービス」＋「そのサービスの追加条件」を結びつけると判断できます。

①まず設問の主語（誰について）と、求める情報の種類（事実／推測）を確認し、「人物の行動・選択」に関する文を探す方針を立てる。  
②手紙・メールなど人物が書いた文書から先に読み、動詞（ordered / chose / requested / complained など）に注目して「何を選んだか・何が起きたか」を線で拾う。  
③次に広告・案内などの文書で、その行動に対応する「条件（料金、日数、追加料金、例外）」を探し、同じキーワード（配送方法など）で対応づける。  
④対応づけたら、「選択した＝その条件が発生する」と論理を1段で言い換えて、設問の “suggested” に耐える推論（確実に言える範囲）に落とす。  
⑤ひっかけは「強い感情語」や「出来事の一部」だけを拾って飛躍させる選択肢（例：ミスがあった→不満、目的が書かれている→不参加）なので、必ず本文の結論文（感謝・満足・今後も利用など）や最終的な解決結果と矛盾しないかで切り捨てる。

次は「人物の選択→別文書の条件」をセットで探し、2つがつながった瞬間に答えを確定させる。感情や目的に引っぱられそうになったら、「本文に書かれた最終評価・結果と一致する推論だけ」を残す。`,
};
...
const tr01An1Q1 = {
  ...
  metacogFeedbackJa: `この設問タイプで最重要なのは、メールの「目的」は冒頭の要点文と結びの行動宣言（謝罪・対応）から一気に特定し、細部の理由説明は後回しにすることです。設問が「なぜ書いたか／目的は何か」と聞くときは、本文全体の機能（通知・依頼・提案・規定説明など）を一語で言い換える問題だと見分けます。今回は、納期が予定より遅れることの連絡が中心です。

①まず件名・冒頭1〜2文で「何の話題で、何が起きたか（変更・問題・依頼・提案）」を拾い、目的候補を2つまでに絞る。  
②次に「予定→実際」のズレを示す表現（最短日程、遅れる、見積もりより後ろ倒しなど）を探し、目的が“状況報告”か“依頼”かを確定する。  
③理由段落（制約、代替案がない等）は「なぜそうなったか」の補強なので、目的そのものと混同しない。  
④複数テキストがあるときは、本文で出た条件（サイズ制限・日付・中止など）を表で照合し、「遅れ／変更が事実として起きている」ことだけ確認して戻る（表の細部を全部読まない）。  
⑤ひっかけは「補償・値引きの話が出た＝目的は割引」と短絡する型なので、結びの対応は“主目的の付随措置”と切り分け、本文で最も長く説明されている問題の通知を優先する。

「目的を聞かれたら、最初に“何が起きたか”を一語で言えるまで読む。理由やお詫び、埋め合わせは主目的の後ろに付く飾りだ」と自分に言い聞かせる。`,
};
...
const tr01An1Q2 = {
  ...
  metacogFeedbackJa: `この設問タイプで最重要な読み方の結論は、「設問の名詞（目的地・日付・金額など）を“本文内の別表現”に言い換えて、複数資料の一致点で確定する」ことです。見分け方は、設問が本文の一文だけで完結せず、メール＋表などの情報を突き合わせないと答えが出ない聞き方になっている点です。今回は「目的地」が、メール側では「到着する場所」、表側では「寄港地・到着地」として現れます。

①設問のキーワードを先に分解し、「何を特定するか（目的地）」「対象は何か（特定の型番の品）」を押さえる。
②メール本文から、対象の品に紐づく“制約条件”を抜く（到着予定日、出港月、積載条件など）→表で探す手がかりに変換する。
③表では、まず積載条件（最大高さなど）で候補行を絞り、次に日付で一致する行を1つにする。
④一致した行の中で、「目的地」に当たる列がどれかを確認し、設問が求める粒度（国なのか都市なのか）に合わせて答えを抽象化する。
⑤ひっかけは「表の最終到着地＝目的地」と決めつけるパターンなので、設問が“最終”を聞いていない限り、メールで示された到着日と一致する地点（途中寄港を含む）を優先して切り捨てる。

「目的地は表の右端とは限らない。メールの条件で表の行を特定し、設問が求める粒度に合わせて場所情報を読み替える。」`,
};
...
const tr01An1Q3 = {
  ...
  metacogFeedbackJa: `表や時刻表の「直行」を問う設問は、出発地と到着地の間にある中継欄が空欄かどうかだけを機械的に確認するのが最重要です。見分け方は、設問にdirectly / without stopping / non-stopのように「途中に寄らない」ことを示す語が入っているときです。本文の説明文に引っ張られず、表の列の埋まり方で判断します。

①まず設問の2地点（出発地・到着地）を表の列見出しで特定し、その2列の間に「経由地の列」がいくつあるかを把握する。  
②各候補の行について、出発地の次の列から到着地の直前までを横にスキャンし、日付が1つでも入っていれば「直行ではない」と即除外する。  
③空欄は「寄港しない」のサイン、いっぽうでcancelledは「その寄港（またはその区間）が成立していない」サインなので、直行判定では「その列に有効な寄港がないか」を確認する。  
④ひっかけは「到着日が早い＝直行」「船名がそれっぽい＝直行」といった印象判断なので、日付の有無だけに基準を固定して切り捨てる。

「直行かどうかは、間の列が全部空かどうかだけで決める。」  
「日付が1つでも挟まっていたら、その時点で直行ではない。」`,
};
...
（D）1つ目の本文の②より、イベントの予定終了時刻。`,
  metacogFeedbackJa: `設問で「提案している開始時刻は？」と聞かれたら、元の予定時刻を別文書で確認し、提案内容の「差分（何分／何時間ずらすか）」を正確に計算して答えるのが最重要です。見分け方は、suggest / recommend / propose などの提案表現と、earlier / later / by ～ などの変更量が設問の核になっているときです。今回は「1時間早く」という差分を、申込書の開始時刻に当てはめます。

①まず設問の主語（誰の発言か）を手がかりに、該当する文書を決めて「提案・依頼・変更」を述べている一文を探す  
②その一文から「変更量（例：1時間早く／30分遅く）」だけを抜き出し、まだ時刻は決め打ちしない  
③次に別文書（申込書・予定表・予約内容など）へ移り、元の開始時刻／締切時刻など“基準になる数字”を特定する  
④基準時刻に変更量を足し引きして、選択肢の時刻と照合する（計算は必ず「開始」なのか「終了」なのかも確認する）  
⑤ひっかけは「本文にそのまま書いてある時刻」を選ばせる型なので、提案問題では“元の時刻”を見つけても即答せず、必ず差分適用後の時刻だけを答える

「提案＝元の予定に対するズレ」を探して、ズレの量→基準時刻→計算の順で処理する。数字が複数出たら、設問が聞く対象（開始／終了／締切）と一致するもの以外は切り捨てる。`,
};
...
（D）1つ目の本文の注文書の⑧の「スタッフが必要」の項目の「はい」にチェックがあるが、server「給仕人」の数の確認は求めていない。clarify「〜を明らかにする」。`,
  metacogFeedbackJa: `この設問タイプで最重要な読み方の結論は、「連絡手段（call / e-mail / visit など）を求める文の直後にある “so that / to / in order to” の目的表現をつかんで、何をするための連絡かを一気に確定する」ことです。設問が「Why does X ask Y to call?」のように理由を聞いてきたら、本文中の依頼文（Please call…）の周辺に答えがほぼ固定で置かれます。今回は「電話＝意思決定の手伝い」という目的が明示されています。

①まず設問の動詞（ask to call）を見て、「連絡を求めた理由＝目的」を探す問題だと決め、本文中の call / contact / reach / let us know などの連絡表現に印を付ける。  
②その文の直後（または同じ文内）の “to不定詞 / so (that) / so we can” を優先的に読む：ここが「電話の目的」を最短で言い切る場所。  
③目的が代名詞（these / it / them）でぼかされていたら、1〜2文だけ上に戻って「these が指す具体物」を回収し、名詞に言い換える（例：料理内容、時間変更、支払い条件など）。  
④複数テキストがあるときは、申込書は「前提情報（希望内容）」、メールは「変更・提案・依頼（行動）」と役割分担して読み、設問が“なぜ連絡するのか”ならメール側の依頼文を主戦場にする。  
⑤ひっかけは「同じ段落にある別トピック（見積もり、支払い、人数、スタッフ等）に目を奪われる」型なので、連絡依頼文の目的表現に書かれていない話題は切り捨てる（書かれているのは“今電話で何をするか”だけ）。

次は「連絡してほしい」と出た瞬間に、その直後の目的表現だけを取りに行く。目的がぼやけていたら、代名詞を具体名詞に戻してから選択肢と照合する。`,
};
...
（D）2つ目の本文の①1～2行目で、値引きされた洗濯機を買おうと新店舗を訪れたことを述べているが、広告キャンペーンに対して感想を伝えているわけではない。feedback「意見、感想」、advertising「広告（の）」。`,
  metacogFeedbackJa: `目的を問う設問は、細部よりも「書き手が何を達成したくてこの文書を書いたか」を、冒頭の状況説明と末尾の締めの一文で挟み撃ちして決めるのが最重要です。見分け方は、設問が「Why did ～ send/write ～?」のように理由を聞き、選択肢が「苦情・称賛・問い合わせ・意見」など“目的ラベル”になっているときです。今回は、本文の最後で感情評価が強く出ている方向に目的が収束します。

①まず設問を「目的当て」だと判断したら、本文の最初の2〜3文で「きっかけ（何が起きたか）」だけを押さえる（買い物・トラブル・依頼など）。  
②次に本文の末尾（結びの1〜2文）を先に読み、「感謝・満足・不満・要望・質問」のどれで締めているかを確認する。  
③中盤は、目的を支える具体例として「相手（誰の行動か）」「行動（何をしてくれた／しなかったか）」を箇条書き的に拾い、②の締めのトーンと矛盾しないかだけをチェックする。  
④ひっかけは、本文中の否定的な語（残念、問題、在庫がない等）に引っ張られて「苦情」と決め打ちするパターンなので、否定語があっても“要求・非難・補償依頼”に発展しているか、最後が不満で終わっているかを条件にして切り捨てる。  
⑤もう一つのひっかけは、本文に出る専門語（credit など）を見て「紛失・問い合わせ」と短絡することなので、その語が「制度の説明」なのか「トラブルの申告」なのかを、動詞（申請した／承認された等）で判定する。

「目的は真ん中ではなく、冒頭のきっかけと結びの一文で決める。否定語や専門語に反応せず、最後のトーンが感謝か不満かでラベルを確定する。」`,
};
...
（A）（B）（C）1つ目の本文の②より、いずれもGrove家電店の店舗がある場所だが、新店舗だとの記述はない。`,
  metacogFeedbackJa: `設問で「どこで会ったか／どこで起きたか」を聞かれたら、本文内の“場所の手がかり”をまず特定し、別テキストにある場所リストと照合して一意に決めるのが最重要です。見分け方は、設問が人の行動（会う・訪れる・開催される）に対して場所を問う形になっていることです。今回は「新しい店舗」という表現を、広告側の店舗情報に結びつけられるかが鍵でした。

①まず設問を「場所特定」だと判断し、本文中で場所を直接言っていそうな名詞（店・支店・会場など）を探す。  
②次に、会った場面の直前直後を読み、「どこで」「いつ」「何のために」という状況語（例：新しい店舗、出張先、イベント会場）を抜き出す。  
③複数テキスト問題では、メールなどの具体描写で得た“特徴語”を、広告・案内などの一覧情報に当てはめて一致する項目を探す（特徴語→リスト照合の流れ）。  
④ひっかけは「本文に並んでいる地名＝正解」と思い込ませる型なので、単なる列挙か、特徴語で限定された場所かを区別し、限定条件がない選択肢は切る。  
⑤最後に、設問が聞いているのは“会った場所”であって“住んでいる場所／配送先”ではない、と行動の舞台に戻して確認する。

「場所問題は、まず本文で“限定条件”を拾い、次に別テキストの一覧に照合して一つに絞る。」  
「地名がたくさん出ても、列挙に引っ張られず、条件で特定できるものだけを選ぶ。」`,
};
...
（D）2つ目の本文の①3行目に、販売員のDawsonさんが店舗で同社製品の動画を見せてくれたことが述べられているのみ。`,
  metacogFeedbackJa: `「最もありそうに正しい」は、本文に明言されないことが多いので、複数テキストの“条件→適用→結果”をつないで論理的に言えることだけを選ぶのが最重要です。見分け方は、設問が事実確認ではなく「most likely true」と推測を求めている言い方になっている点です。今回は、メール内の行動と広告の制度条件を結びつけると一つだけ自然に確定します。

①まず設問を「人物についての推測＝その人が実際にした／するはずの行動・状態」を問うものだと捉え、選択肢の動詞（連絡する・支払う・売る・見る）に下線を引いて“検証すべき行動”を固定する。
②次にメールを先に読み、本人がしたこと（申請した、承認された、見せてもらった等）と、まだ起きていないこと（到着はこれから等）を分けてメモする。
③そのうえで広告に戻り、制度やサービスの「利用条件（〜した人が使える）」を探し、メール側の事実がその条件を満たしているかを照合する。
④照合が一致したら、「条件を満たす＝通常その前提行動も行っている」と言える範囲だけを推論し、逆に“書かれていない未来の予定”や“別の可能性もある行動”は切り捨てる（典型的なひっかけは、本文に出た人物名・動画・来月などの語に引っ張られて根拠のない予定を選ばせるもの）。
⑤最後に、選択肢を「本文の事実＋制度の条件からほぼ必然」か「単なる想像」に二分し、前者だけを残す。

「most likely true は、本文の断片をつないで“必然に近い推論”だけを選ぶ。未来の予定や気持ちの推測は、根拠が一語でも欠けたら捨てる。」`,
};
...
（D）pose a risk to～「～に対して危険をもたらす」。`,
  metacogFeedbackJa: `設問の主語（ここでは特定の対象）について「本文が何を言い切っているか／どこまで推測してよいか」を、評価語（estimate, believe, recommend など）と期間・条件のセットで拾うのが最重要です。見分け方は、What does the report suggest about ～? のように「示唆」を聞いていても、本文中の断定・推定・提案のどれを根拠にするかが勝負になる点です。今回は「継続可能な年数」と「利益が出るか」の組み合わせが核になります。

①設問の対象語を丸で囲み、本文中でその対象が出る段落だけを先に探して読む（他の対象の段落は後回し）。  
②その段落では、数字（年数・距離・深さ）と評価動詞（estimate / indicate / believe など）に線を引き、「条件（現状のペースなら等）→見積もり（何年）→結論（採算性）」の順で要点を1本の因果でつなぐ。  
③選択肢は、本文の「言い切りの強さ」と一致するものだけ残す（recommend があるなら「すべき」、believe/estimate なら「〜だろう」寄り、など）。  
④ひっかけは「本文にある単語に引っ張られるが、主張が追加されている」型（例：コスト増→だから急いで採掘すべき、のような飛躍）なので、本文に命令・提案表現がない選択肢は切る。  
⑤最後に、残った選択肢が「対象についての要約」になっているか確認し、別の話題（追加調査・危険性など本文未言及）にずれていれば除外する。

次は「対象の段落だけを拾い、条件つきの見積もり＋結論のセットで答える」と自分に言い聞かせる。本文にない提案やリスクを、単語の連想で足さない。`,
};
...
（C） Springbokコンクリート供給合同会社の社名は①の「依頼者」欄にあるので、この2人が同社のオーナーだとは考えられない。`,
  metacogFeedbackJa: `この設問タイプで最重要な読み方の結論は、「人が誰か」を問われたら本文内容より先に文書の体裁（見出し・依頼者・作成者）から所属を確定することです。見分け方は、設問が職業や立場を直接たずね、選択肢が「現場側／依頼側／作成側」のように役割で分かれているときです。本文末尾の署名欄は、根拠が最短で出る場所になりやすいです。

①設問を見たら、まず「誰＝所属・役割」を決める問題だと判断し、本文の署名・作成者・発行元・依頼者の欄を探す。  
②発行元（報告書の看板）と依頼者（お金を払って頼む側）を区別して読み、どちらの立場の文書かを先に確定する。  
③名前が出た位置のラベル（作成者／担当者／連絡先など）をそのまま役割に変換し、本文中の作業描写（調査した、分析した等）は「作成側の仕事」として補強に使う。  
④ひっかけは、本文に出る作業内容から「現場で働く人」と短絡するパターンなので、「その人が文書を作っているのか／作業を依頼されているのか」をラベルで判定して切り捨てる。  

次は「名前＝本文の中身」ではなく「名前＝文書の枠（発行元・依頼者・作成者）」で決める。迷ったら、署名欄と依頼者欄のどちらに近い名前かだけを見て所属を確定する。`,
};
...
const tr02Question3 = {
  ...
  metacogFeedbackJa: `挿入文問題で最重要なのは、前後の文と「指示語・同じ名詞・評価語（良い/悪い）」が自然に連鎖して、論理の流れ（原因→判断→提案）が途切れない位置を選ぶことです。見分け方は、設問が「どこに入るか」を聞き、挿入文に代名詞や評価（low/high, problem/benefit）など“つなぎ役”の要素があるとき、前後の因果や対比の接着剤として働く場所を探すタイプだと判断します。今回は「残っている」という語が直前の「残量」の話を受ける必要がある、という一点をまず押さえます。

①挿入文の中のキーワード（残っている／品質が低い）を丸で囲み、「何が残っているのか」を直前で明示している文を探す。  
②候補位置の前後1〜2文だけを見て、挿入文が「追加情報（マイナス点の上乗せ）」として自然か、「結論（提案）に向けた根拠の補強」になっているかを確認する。  
③接続語（therefore, in contrast など）がある文の直前は要注意で、そこはたいてい「結論に入る直前の根拠置き場」なので、挿入文が根拠として機能するなら優先的に当てはめる。  
④ひっかけは「同じ単語（gravel など）が出ているから」という表面一致で、対比パート（別の場所の話）や将来予測パートに入れてしまうこと；挿入文が“どの対象”を指すかがズレるなら即切る。  

「挿入文は、直前の名詞を受けて具体化し、直後の結論へ橋渡しできる場所に置く。」  
「同じ単語があるだけで飛びつかず、原因→評価→提案の順番が崩れないかで決める。」`,
};
...
（D）①の1行目にMoriさんがカナダの大学を卒業したとあるが、創業者の出身地についての言及はない。`,
  metacogFeedbackJa: `この設問タイプで最重要な読み方の結論は、「会社について何が示されるか」は“評価”ではなく本文中の客観情報（事実）を、特に現在の状況・範囲・実績を述べる文から拾うことです。見分け方は、設問が「indicated / stated / mentioned」などで本文根拠の事実確認を求め、選択肢が「どこで・何を・誰が」など具体情報になっているときです。今回は「どの地域で展開しているか」という地理的な範囲の事実が決め手になります。

①設問の名詞を言い換えて狙いを絞る（「会社について」＝所在地ではなく、事業の特徴・展開地域・製品ライン・実績などの“会社情報”）。  
②本文は時系列の伝記パートと、会社の現状をまとめるパートに分け、会社の特徴が出やすい「現在・継続・成長」を示す表現（例：today, continues, sales grow）を優先して当たる。  
③「場所・範囲」を問う選択肢があるときは、地名そのものよりも「and / throughout / in addition / while」などの接続で“複数地域にまたがる”構造になっている文を探して、範囲を広げて読む。  
④ひっかけは「一部を全体にする」か「本文にない断定」を混ぜる（例：特定の製品名が出た→それ“だけ”作る、留学先が出た→出身地も同じ、など）。本文が“限定語（only など）”や“頻度（often など）”を本当に言っているかを必ず確認し、なければ切る。  

「indicated系は、本文に書かれた事実だけを拾う。現在の状況をまとめる文と、範囲を広げる接続語を最優先で見に行く。」`,
};
...
const tr02An1Q2 = {
  ...
  metacogFeedbackJa: `挿入文問題で最重要なのは、代名詞・指示語と因果（発見→その後の行動）のつながりが最も自然に連結する位置を探すことです。設問が「どこに入るか」を聞き、文中に they / this / such などがあるときは、直前直後の「誰の話か」「何が起きた結果か」を照合して決めます。今回は「発見した」という出来事が、その後の展開（やりとりが続く等）を生む位置に置けるかが鍵です。

①挿入文の中の手がかり語を先に抜き出す（代名詞の指す相手、時制、感情・評価語、discover などの出来事語）。  
②各候補位置の直前で「登場人物が複数そろっているか」「会話・出会いなど“共有点が判明しそうな場面”か」を確認する。  
③挿入文を入れた直後の文が、挿入文を受けて自然に続くかを見る（発見→その後の交流・協力・決断、のように因果がつながるか）。  
④ひっかけは「同じテーマ語が近くにあるから置きたくなる」パターンなので、テーマ一致だけで決めず、代名詞の参照先と出来事の順序（発見が先／結果が後）で不自然なら切る。  

「代名詞が誰を指すか、まずそこを固定してから場所を選ぶ。」  
「挿入文は前後を“原因→結果”でつなぐ接着剤になる位置にしか入らない。」`,
};
...
（D） no longer「もはや～ない」。`,
  metacogFeedbackJa: `設問の「なぜ（why）」は、本文中の「会社側が明言する目的・狙い」を探して、その一文を言い換えた選択肢を選ぶのが最重要です。特に社名変更・方針転換のニュース記事では、発表→理由（引用）→補足データの順で理由が置かれやすく、引用部分が根拠の中心になります。今回は「どう見られたいか」という発言が、そのまま理由の核です。

①設問を先に読み、「理由＝目的（to / in order to / want / aim / help）」を探すモードに切り替える。
②本文は冒頭で「何が起きたか（変更内容）」を押さえたら、次に来る引用・コメントを最優先で精読し、「会社が望む印象の変化」を一言で要約する。
③その要約を、選択肢の抽象語（image / reflect / represent など）に対応づけて照合する。
④ひっかけは「本文に出た単語に引っ張る」タイプ（例：短くする＝覚えやすい、特定商品＝もう売っていない）なので、単語一致ではなく「目的として言い切っているか」を基準に切り捨てる。
⑤数字やターゲット層などのデータは理由の補強であって主理由ではないことが多いので、まず引用の目的文で答えを確定し、データは確認に回す。

次は「why を見たら、会社のコメントの want/aim を探して一文要約し、それと同じ抽象度の選択肢を選ぶ」と自分に言い聞かせる。単語が似ているだけの選択肢ではなく、「目的として断言されている内容」だけを拾う。`,
};
...
（C） audience「視聴者、観客」。`,
  metacogFeedbackJa: `設問の動詞が「hope to accomplish / aim / intend」系のときは、人物起用・施策の“目的（ターゲット＋狙う変化）”を、直前直後の因果つながりから拾うのが最重要です。特に「採用した／起用した」などの行動が出たら、その理由を説明する文（to不定詞・in order to・so that・help (to)）がほぼ答えになります。今回は「誰にアピールしたいか」を示す表現が鍵でした。

①まず設問で「何を成し遂げたいか＝目的」を聞いていると確認し、本文では“行動（起用・変更・開始）”が書かれている箇所を探す。  
②行動の直後に出る目的表現（to不定詞、help (to)、so that など）に下線を引き、「何を増やす／変える」かを動詞で押さえる。  
③目的文に代名詞（that/this/those/it など）や「その層」系が出たら、必ず直前に戻って指示先＝ターゲット属性（年齢・性別・顧客層・売上比率など）を特定する。  
④選択肢は「ターゲット」と「変化の方向」が一致するものだけ残し、本文にない“有名人側の利益”や“番組・イベントの話”にすり替わるものは、関連語が出ていても目的がズレるので切る。  

「起用・変更のニュースが出たら、次の一文で目的を取る。代名詞や『その層』が出たら、必ず一文戻って具体的な対象に言い換える。」`,
};
...
be set to do「〜する用意ができている」。`,
  metacogFeedbackJa: `挿入文問題で最重要なのは、挿入文の「話題（名詞）」と「機能（追加説明・理由・具体化・時系列）」を先に特定し、その直前直後で同じ話題が自然につながる場所を探すことです。見分け方は、設問が「どこに入るか」を聞き、本文中に複数の挿入位置が示されているときです。今回は「広告」「その開始時期」という情報が、ある人物の起用の話題にぶら下がる形で置ける場所が正解になります。

①まず挿入文を単独で読み、「主語は何か／何についての文か／いつの話か」をメモする（例：広告＋開始タイミング＝時系列の補足）。  
②本文を上からざっと見て、挿入文のキーワードと同じ話題が初めて出る地点を探し、そこより前の候補は切る（初出より前に置くと読者が「それ何？」となる）。  
③次に、その話題が続いている段落内で「一般論→具体例」「発表→詳細」「期待→運用（いつ始まる等）」の流れになっている箇所を探し、挿入文がどの役割として最も自然かを判定する。  
④ひっかけは「同じ単語があるから」という表面一致で早い位置に入れてしまうことなので、挿入文が指す対象が直前文だけで一意に特定できるか（代名詞・固有の話題の連続）を必ず確認してから決める。  

「この文は何の話題にぶら下がる補足で、前後の文のどちらを受けてどちらへ渡すのか？」と自分に問い、話題の初出前や話題が切り替わる境目には置かない。最後に、挿入後の2文を続けて読んで“時系列と説明の流れ”が滑らかかで確定する。`,
};
...
（D） expand「拡大する」。`,
  metacogFeedbackJa: `設問が「案内文から何が分かるか／示唆されるか」と聞くタイプでは、細部の数字や条件より先に、冒頭と結びにある“全体方針（この文書が何をアピールしているか）”をつかむのが最重要です。見分け方は、設問が理由や金額ではなく「suggested / can be inferred」のように本文全体から言える一般化を求めているときです。こういうときは、個別の制度説明は「方針を支える具体例」として読むと、選択肢の言い換えに強くなります。

①まず冒頭段落と最終段落だけを先読みし、「会社の姿勢・目的」を表す抽象語（例：顧客重視、迅速、公正、透明など）に線を引く。  
②次に各小見出し（請求・予約・停電など）を見て、「共通して何を保証しているか」を一言でまとめる（時間厳守、事前通知、遅延時の補償のように）。  
③選択肢は「本文の中心テーマに合う抽象度」かでふるいにかけ、テーマと無関係な“新情報”っぽい語（設備更新、エリア拡大、延滞料など）が出たら、本文にその話題カテゴリ自体があるかを確認して即切る。  
④残った候補は、本文中の同義表現（高評価＝質が高い、常時対応＝サービス重視、保証＝顧客満足の裏付け）に置き換えて一致するかをチェックする。  
⑤最後に、数字・条件（何時間、何日、いくら）に引っ張られて「細部が書いてある＝正しい」と誤認しないよう、設問が求めるのは“全体として言えること”だと再確認する。

「まず冒頭と結びで、この文書が何を売りにしているかを一言で言えるようにしてから、細部はその証拠として読む。」  
「選択肢に本文にない話題が混ざっていたら、具体的でも即アウトにする。」`,
};
...
typically「通例、一般的には」。`,
  metacogFeedbackJa: `挿入文問題で最重要なのは、挿入文の「話題（何について）」と「つながり方（前後の語の受け渡し）」が最も自然になる場所を、段落の機能ごとに当てはめて決めることです。見分け方は、挿入文にオンライン・時間・手続きなどの具体要素があり、前後にそれを受ける語（方法→所要時間→例外対応など）が並ぶかどうかを見ます。今回は「オンラインでの依頼→返答までの時間」という流れに乗る位置が正解になります。

①まず挿入文を先に読み、「主語は誰か／条件は何か／言いたい結論は何か」を1行で要約する（例：オンラインの場合の標準処理時間）。  
②次に本文側で、同じ話題の段落を探し、見出し語やキーワードで範囲を絞る（請求・予約・停電など、カテゴリが違う段落は即除外）。  
③候補段落内では、「手段の提示→標準の所要時間→例外（追加調査）→補償」のような情報の並びを意識し、挿入文がどのスロットに入ると最も自然かを確認する。  
④ひっかけは「数字・時間」に目が引っ張られて、別の段落の時間表現（到着枠や事前通知）に入れたくなるパターンなので、数字ではなく“何の時間か（返信／到着／通知）”が一致しているかで切り捨てる。  
⑤最後に、挿入位置の前後を続けて音読するつもりで読み、指示語や語彙の重複が不自然（同じ説明の二重化、話題の急転換）になっていないかで確定する。

「挿入文は内容一致だけでなく、段落内の役割の順番に置く。数字に釣られず、“何についての数字か”を必ず一致させる。」`,
};
...
（D）②に、Leffleyさんは面接中に今週国外にいる予定だと言ったとあるが、面接の予定を変更したとは述べられていない。reschedule「～の予定を変更する」。`,
  metacogFeedbackJa: `設問が「〜について何が示されているか」と人物情報を聞くときは、会話の中でその人物が「どの集団に属する人か（新規採用者／担当者など）」を先に確定し、そこから確実に言える事実だけを選ぶのが最重要です。特にメッセージ形式は、主語が省略されたり話題が飛んだりするので、「誰のことを指している代名詞・言い換えか」を追う読み方が効きます。今回は「欠席する1人」が誰かを特定できれば、その人物の立場（これから働く側）まで一気に固まります。

①設問を見たら、人物名に印を付け、「身分（新規／既存）」「予定（参加／不参加）」「評価（経験／性格）」のどれが問われそうか当たりをつける。  
②本文は最初に全体状況（研修・採用・人数など）を押さえ、次に人物が出る文だけを拾って「その人物＝どの枠の人か」を結びつける。  
③「欠席者」「その人」「彼女」などの指示語・言い換えを、直前の名詞に機械的に戻して同一人物か確認する（ここがズレると全滅する）。  
④選択肢は、本文にある語と似た語（例：本社、面接、経験など）に引っ張られやすいので、「本文に書かれた事実」か「それっぽい推測」かを分け、推測しか支えがないものは切る。  
⑤最後に、確定した属性から言えることだけを残す（予定の変更・会議参加など、本文に明示がない行動は選ばない）。

「人物問題は、まずその人がどのグループの誰かを固定してから、本文に明示された事実だけを拾う。」  
「似た単語が見えたら飛びつかず、指示語を戻して同一人物かを必ず確認する。」`,
};
...
（A）①で、指導係は明日の研修会に来る予定だと言っている。currently「現在、目下」、unavailable「都合がつかない」。`,
  metacogFeedbackJa: `発言意図を問う設問は、引用文だけを訳すのではなく「直前の情報→その反応→直後の返し」で会話の因果をたどって、提案・推測・確認のどれかを確定させるのが最重要です。見分け方は、設問に「most likely mean」「when he writes」のように“その一文の真意”を聞く形が出たときです。今回は「来週また来るべき」という提案が、欠席者の事情を受けた調整案だと読めます。

①まず引用文の機能を決める（提案／依頼／確認／反論など）：助動詞や「〜すべきだ」の形は、相手への提案になりやすい。  
②引用文の直前で「問題の種」を特定する：誰が何に参加できないのか、理由は何か（予定・場所・時間など）を拾う。  
③引用文がその問題をどう解決しようとしているかを言い換える：「欠席者がいる→別日程で同じ機会を用意する」という因果でまとめる。  
④直後の返答で検証する：相手が「不要」「別案がある」などと返すなら、引用文は“解決策の提案”だったと確定できる。  
⑤ひっかけは「文中に出た単語に引っ張る」型：たとえば不在・準備不足・休暇など、会話に直接書かれていない理由を選択肢がそれっぽく言い換えるので、「本文に明示された原因（欠席者の都合）と一致するか」だけで切り捨てる。

「この一文は単体で訳さず、直前の問題と直後の反応で“何を解決したい提案か”に直す。」  
「本文にない理由をそれっぽく足した選択肢は、会話の因果に合うかで即除外する。」`,
};
...
（D）報告書の修正については話題に上っていない。revise「～を修正する」。`,
  metacogFeedbackJa: `「update／変更があった」と言われたら、直後の文で“何がどう変わったか”を特定し、設問の言い換えとして選択肢に落とし込むのが最重要です。こういう設問は「その表現は何を意味するか」と聞きつつ、実際は前後の具体情報（期限・予定・条件など）の変化を拾えるかを見ています。今回は直後に“提出に関する条件”が更新された説明が続くので、そこに焦点を当てます。

①まず設問の引用部（update）を「何かが前提から変わった合図」と捉え、直後1〜2文を最優先で読む。  
②直後の文から「変わった対象」を名詞で抜き出す（例：期限、場所、担当、手順、金額など）→次に「変化の方向」（早まる／遅れる／追加／中止）を動詞で押さえる。  
③その変化が“誰の都合で起きたか”や“いつ知らされたか”は補助情報として扱い、設問が聞いている「updateの中身」そのものと切り分ける。  
④選択肢は、本文に出た単語に引っ張られず「対象＋方向」が一致するものだけ残す（典型的なひっかけは、本文に出た別トピックの語を混ぜて“それっぽく”見せるものなので、対象がズレていたら即切る）。  
⑤最後に、引用部の直前で共有されていた前提（例：期限はいつだと思っていたか）と、update後の前提を対比し、「前提が更新された点」を一文で言い換えられたら確定する。

「updateと言われたら、直後の具体説明から“何がどう変わったか”を名詞＋動詞で取る。」  
「本文の単語の一致ではなく、前提の差分（変更点）だけを選ぶ。」`,
};
...
（D）①で、アップロードができないというトラブルへの言及があるのみ。technical「技術上の」。`,
  metacogFeedbackJa: `人物推定の設問は、「その人が何をした／言ったと周囲が参照しているか」を手がかりに、発言のつながり（指示語・言い換え・同一話題）をたどって特定するのが最重要です。見分け方は、設問が「Who most likely…?」のように本文に肩書きが直書きされず、会話内の関係性から推測させる聞き方になっている点です。今回は「送ったメール」の参照関係をつなぐと一意に決まります。

①まず固有名詞が出たら、その直前直後で「その人がした行為（依頼・連絡・決定）」を探し、動詞をメモする。  
②次に、別の発言に出てくる「そのメール／その依頼／その変更」などの指示表現を見つけ、同じ出来事を指しているかを照合する。  
③照合できたら、「誰が誰に」その行為をしたのか（送信者・受信者・指示を出す立場）を会話の役割関係で埋める。  
④ひっかけは「話題の分野にいそうな職種」や「最初に出たトラブルに関係しそうな担当」を選ばせる型なので、職種連想ではなく“参照されている行為の主語”が本文で一致するかだけで切り捨てる。

「人名が出たら、肩書き探しではなく“その人がしたこと”を一本の線でつなげ。」  
「分野のイメージに引っ張られたら、指示語が指す出来事の主語に戻って確認する。」`,
};
...
（D）④で、資材の見積もりが未完了だと述べているだけ。be unsure about〜「～について確信がない」。`,
  metacogFeedbackJa: `心配の中身を問う設問は、感情語そのものより「未完了・未対応・間に合わない」を示す発言（need to／still／can’t など）を拾って言い換えるのが最重要です。見分け方は、設問が concerned / worried / problem / trouble などで「何が問題か」を聞いているときで、会話の中の“障害になっている作業”が答えになります。今回は「まだ〜する必要がある」という未完了の作業がそのまま心配の核です。

①設問を見たら「心配＝何がボトルネックか」を探すと決め、会話では不安・困難・未完了を示す表現に印を付ける。  
②発言者を固定して、その人の文だけを縦に追い、「まだ」「必要」「できない」「間に合わない」などの後ろに来る作業名を抜き出す。  
③締切変更・会議予定など周辺情報は、本人の未完了タスクと因果でつながるかだけ確認し、つながらないものは捨てる。  
④選択肢は「本文にある事実の言い換え」か「推測の追加」かで切り分け、見ていない＝削除した、会議がある＝出席できない、のような飛躍は即除外する。  
⑤最後に、抜き出した作業名が選択肢では動詞句（finish / complete / finalize など）に言い換えられていないかを照合して決める。

「心配は感情じゃなく、未完了のタスクとして言い換える。」  
「予定やメールの話に引っ張られず、本人が“まだ必要”と言っている作業だけを答えにする。」`,
};
...
（D）Dengさんは⑤で、早めに念を押さなかったことを謝っているが、TillmanさんがDengさんに確認や念押しを目的としたリマインドメールを送るとは述べていない。reminder「思い出させるもの」。`,
  metacogFeedbackJa: `結論として、「代名詞（it/this/that）＋あいまい動詞（do/handle/see to）」が出たら、直前の“未完了の用件”に指すものを戻って特定するのが最重要です。設問が「この発言は何を意味するか」「most likely mean」と聞くときは、辞書的な意味よりも会話の流れ（依頼→引き受け）で解釈します。今回は「やっておく」が、直前に出た作業の引き受けを表しています。

①設問の引用部分を見たら、まずその文の中の代名詞や省略（it など）に印を付ける。  
②引用文の1〜2通前に戻り、「今やるべき作業」「変更・連絡・確認などのタスク」が提示されている箇所を探す。  
③そのタスクが「まだ完了していない／これから対応が必要」かを、時刻・状況描写（もう閉めた等）と合わせて判断する。  
④「引き受け表現」（I’ll…, Don’t worry, I can…, I’ll take care of it など）が来たら、直前タスク＝it と機械的に対応させる。  
⑤ひっかけは「同じ場面の別行動」や「それっぽい善意の行動」を選ばせる形なので、本文に“その行動をする”と明示がないものは、単語が近くても切り捨てる。

「it が何か」を当てる問題は、直前の未処理タスクに戻って一本化する。雰囲気で“親切そうな行動”を選ばず、本文に書かれた作業だけに結び付ける。`,
};
...
（D）⑦より、TillmanさんがDengさんの頼み事を聞いたことが分かる。それに対してDengさんはお返しをしたいと発言しているのみ。do～a favor「〜の願いを聞く、～に便宜を図る」。`,
  metacogFeedbackJa: `この設問タイプで最重要なのは、人物についての「示唆」は会話の雑談部分ではなく、役割・予定・シフトなどの事実情報から逆算して確定することです。見分け方は、設問が「suggested / implied / can be inferred」などで人物像を聞いてきて、本文にそのままの一文が出にくいときです。今回は勤務時間に関する言及が決め手になります。

①設問を見たら、まず「人物の何を推測させたいか」をカテゴリ化する（勤務・責任範囲・予定変更・人間関係など）。  
②本文では、時刻・曜日・シフト・担当などの数字や業務語を優先して拾い、誰に紐づく情報かを線で結ぶ。  
③「ありがとう／助かった／代わってくれて」などの表現が出たら、直後の目的語（何を代わったか）を必ず確認し、そこから通常の担当を推定する。  
④ひっかけは「本文に出た単語をそのまま選択肢に当てはめる」型なので、開館時刻などの情報は“誰の担当か”が本文で言われていない限り、人物の責任に飛躍させない。  
⑤最後に、推定した内容が「本文の事実1つで必ず支えられるか」をチェックし、支えが弱いもの（推測の推測）は切る。

「示唆」は気分や印象ではなく、数字・担当・依頼のやりとりから固める。単語が一致しても、主語が一致していなければ別情報だと疑う。`,
};
...
（D）⑨で、資格があればできる作業としてソース作りが挙げられているのみ。server「給仕人」。`,
  metacogFeedbackJa: `時刻や曜日を聞く設問は、会話全体を「予定表」に変換して、誰が・いつ・何をするかの確定事項だけを拾うのが最重要です。見分け方は、設問に tomorrow / next week / on Friday など具体的な時間語が入っていて、「何が起こるか」と出来事を問う形になっていることです。こうすると、明日の出来事は「開始が合意された行動」に絞って判断できます。

①設問の時間語を先に丸で囲み、その日付に関係する発言だけをチャット内で上から探す（開始日・依頼・承諾が出やすい）。  
②「できる／かもしれない」ではなく、「依頼→了承」「予定→確定」になっている文を優先してメモする（will / can start / can you…? → sure などの流れ）。  
③同じ話題でも「明日」と「別の曜日」が混ざるので、曜日ごとに出来事を分けて整理し、設問の曜日以外は一旦捨てる。  
④ひっかけは、本文に出た名詞（休暇・箱・資格・ソースなど）だけで選択肢を作り、日付や主体がズレているパターンなので、「その行動が明日に実行されると本文で確定しているか」を最後に必ず照合して切り捨てる。

「時間を聞かれたら、会話を予定表にして“その日に確定して起こること”だけを選ぶ。」  
「単語が出てきたからではなく、依頼と了承（または開始日）が揃っているかで判断する。」`,
};
...
（D）直前のPlebaniさんの発言で、資格があればできる作業として生地作りに言及があるだけで、Berkさんがこの時点で生地を作る人を探すことを意図しているとは考えられない。`,
  metacogFeedbackJa: `発言の意図を問う設問は、引用文だけを訳すのではなく「直前の依頼→その返答→直後のフォロー」という会話の因果で読むのが最重要です。見分け方は、設問が “most likely mean” のように「真意・含み」を聞いていて、引用符つきで一文だけ抜き出しているときです。結論としては、引用文はたいてい「依頼に対する可否（できる／できない）」を遠回しに伝える返答になります。

①まず引用文の直前を見て、相手が何を頼んだ／提案したのか（依頼内容＋日時）を1行で押さえる。  
②次に引用文を「その依頼に対する返事」として読み、肯定か否定かを判定する（休み・不在・予定がある＝実質的な否定になりやすい）。  
③続けて引用文の直後を確認し、別の人が代案を出す／引き受ける／謝るなどの反応があるかを見る（ここが意図の確定材料）。  
④選択肢は「会話に出た単語に引っ張られる」ひっかけを切る（休み＝長期休暇、資格＝別作業、など話題の単語を広げすぎた解釈は捨てる）。  
⑤最後に、答えは「引用文が果たしている会話上の機能（断り・確認・提案）」に一致しているかで決める。

「この一文は単独の意味じゃなく、直前の依頼への返事として読む。直後に誰がどう動いたかまで見て、結局“できる／できない”のどちらを伝えたのかで選ぶ。」`,
};
