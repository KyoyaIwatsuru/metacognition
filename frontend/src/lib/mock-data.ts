...
（C） invite～to do「～に…するよう招く」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「目的を聞く問題」** です。まず手紙の種類を見て、 **「何のために書いたか」** を判断します。
文書の種類が **「手紙」** のときは、用件は **「苦情／感謝」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「なぜ書いたか」なので、答えは **主目的を1つ** にすると決める。
2. 手紙を先に見て、 **冒頭と末尾** で「お願いしている／困っている／感謝している」などのキーを押さえておく（見る場所を決める）。
3. 手紙の中盤で、話が **問題の説明→対応への評価・感謝** に変わって、どちらに収束しているかを確認する。
4. ひっかかりやすい所： **冒頭のトラブル説明だけ** を目的だと決めて「苦情・懸念」側に寄ること。→目的は **末尾の結論** で判断し、途中の出来事は捨てる。
5. 必要なら広告は、手紙に出てくる **サービス種別などの事実** が合っているかだけを軽く照合し、目的判断の中心にしない。

選択肢のチェック（最後に）
✅ 目的判定を「手紙の末尾の結論表現」で確定し、途中の出来事は補助情報として扱ったか（Yes/No）？`,
};
...
（C） be unable to do「〜することができない」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「複数の文書を見比べる問題」** です。まず **設問（suggested の推論の強さ）** を見て、 **「本文から言えること」** を判断します。
文書の種類が **「広告／手紙」** のときは、用件は **「条件の案内／出来事の報告」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問の **suggested/indicated** を見て、「書いてある事実」か「書いてないが言えること」かを先に決める。
2. 手紙の最初の段落で、 **選んだ配送方法** と **届け先として書いた場所** を拾い、選択肢の語（場所／不満／行けない／金額）に対応づける。
3. 広告で、同じ配送方法の **追加料金** を確認する（サービス名 → 料金の対応づけ）。
4. 手紙の「その配送方法を選んだ」＋広告の「その配送方法は追加でこの金額」を **合体** して、金額の選択肢が成り立つか判断する。
5. ひっかかりやすい所： **届け先の場所** を見て「住んでいる場所」と早とちりする／トラブルがあっただけで「不満」と決めつける。→ 住所欄と、最後の感謝・再利用の言葉で捨てる。

選択肢のチェック（最後に）
✅ 数字が手紙に出ていなくても、「手紙の配送方法」と「広告の追加料金」を同じものとして結びつけて説明できますか？（Yes/No）`,
};
...
const tr01An1Q1 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
これは **「目的を聞く問題」** です。まず件名と冒頭を見て、 **「何のために書いたか」** を判断します。
文書の種類が **「取引先へのメール」** のときは、用件は **「変更の連絡／謝罪」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「なぜ書いたか」だと確認し、選択肢は **説明／依頼／提案／方針** のように「言いたいことの種類」として見る。
2. 件名→冒頭で、 **何について連絡しているか** と、 **最初に出る問題・変更（遅れ等）** を押さえる。
3. 本文中盤で、 **問題発生＋影響（遅れ・変更）** を拾い、 **何が起きたか** を理解する。
4. 結びで、 **謝罪・補償・次の対応** を見て、全体が **通知・説明** なのか、 **依頼・提案** なのかを決める。
   - ひっかかりやすい所： **補償や値引き** が出ると「提案が目的」と思いがちだが、ここは **主目的の後のフォロー** として切り、最初と最後で一致する用件を優先する。
5. 途中の理由（不運な出来事、条件の制約など）は **主目的の補強** として扱い、主目的そのものと取り違えずに、いちばん強く繰り返される中心行為に合うものを1つ選ぶ。

選択肢のチェック（最後に）
✅ 冒頭の用件提示と結びの謝罪・対応が同じ主目的（通知・説明）に収束していると確認できたか（Yes/No）？`,
};
...
const tr01An1Q2 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
これは **「複数の文書を見比べる問題」** です。まずメールから **表に当てられる手がかり（日時など）** を見て、「どこに届く話か」を判断します。
文書の種類が **「メール＋スケジュール表」** のときは、用件は **「遅れ連絡／日程確認」** になりやすいので、まず日付を拾って表で確かめます。

解答への手順

1. 設問を言い換えます： **「目的地＝どの国か」** を答える、と決める（場所名を拾う問題）。
2. メールの中盤から、 **到着できる最短の日付** と、 **参照すべき資料がスケジュール表** だと分かる手がかりを押さえる。
3. 表で、その **最短日付と同じ日付** があるセルを探し、 **どの列（寄港地）か** を特定する。
4. その列の **見出し（都市／国）** を読み、 **選択肢の国名** に合わせて答える。
5. **ひっかかりやすい所：** 表の **いちばん右の列＝目的地** と決めつけて選ばないこと。設問の destination が「最終目的地」なのか「その日付で到着する地点」なのか、 **日付が一致した列** で判断する（範囲外の列は切る）。

選択肢のチェック（最後に）
✅ メールの最短日付→表の同一日付セル→その列見出し（国）という対応づけを1本の線で説明できるか（Yes/No）。`,
};
...
const tr01An1Q3 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
これは **「表を使う問題」** です。まず表の見出しを見て、 **「どの列をどう満たせば条件クリアか」** を判断します。
文書の種類が **「スケジュール表」** のときは、用件は **「予定の確認／条件に合うもの探し」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問の条件を言い換える： **directly＝間の列（中間の寄港地）に全部「寄らない」** こと。
2. 表の見出しで、 **Canadaの列・Japanの列** と、その **間にある中間港の列** を押さえる。
3. 各船は **同じ行を左→右** に追い、 **Canada列とJapan列に日付があるか** を確認する（片方だけは不可）。
4. 次に、同じ行の **中間港の列** がすべて **空欄／記号／中止** など「寄港なし」扱いかを確認し、 **1つでも寄港があれば切る** 。
   - ひっかかりやすい所： **directlyを「最短・最速」と勘違いして日付だけで選ぶ** ／ **空欄・記号・中止の意味を混ぜて判定を取り違える** 。→必ず「間の列が全部どうなっているか」で判断する。
5. 条件を満たす行が1つに絞れたら、その船名で確定する（根拠は「間の列が全部非寄港」）。

選択肢のチェック（最後に）
✅ Canada→Japanの間にある中間港の列を全て確認してから直行判定したか（Yes/No）？`,
};
...
（D）1つ目の本文の②より、イベントの予定終了時刻。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「複数の文書を見比べる問題」** です。まず設問の主語（誰の提案か）と start の対象を見て、 **「どの文書の情報を基準にするか」** を判断します。
文書の種類が **「注文書／メール」** のときは、用件は **「変更の連絡／提案」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問で「提案された開始時刻」を聞いていると押さえ、設問の人物（発言者）はメール送信者だと特定する（設問の範囲を先に決める）。
2. メール冒頭段落から、提案の言い方＋変化量（早く/遅く、何時間/何分）を拾う。
3. 別文書（表）で、元の開始時刻がどこにあるかを見つける（時間欄の開始側）。
4. 元の開始時刻に変化量をそのまま足し引きして、新しい開始時刻を出す。
5. 選択肢と照合し、表記まで合わせる。 **ひっかかりやすい所：** 「早く」を終了時刻や所要時間に当ててしまう／メールだけで完結すると決めつけて元の時刻を見ない → **開始時刻の欄だけ** を見て、範囲外（終了時刻など）は切る。

選択肢のチェック（最後に）
✅ 変化量（早く/遅く）と元の開始時刻の両方を根拠箇所から取って計算したか？（Yes/No）`,
};
...
（D）1つ目の本文の注文書の⑧の「スタッフが必要」の項目の「はい」にチェックがあるが、server「給仕人」の数の確認は求めていない。clarify「〜を明らかにする」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「目的を聞く問題」** です。まず **依頼の言い方（電話して、など）** を見て、 **「何のためにしてほしいか」** を判断します。
文書の種類が **「メール」** のときは、用件は **「確認のやり取り／提案」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問を **「なぜ電話してほしいのか＝電話の目的」** に言い換え、答えを「電話で何をするためか」に限定する。
2. メール本文で **call / please call** を目で探して注目し、 **その前後1〜2文だけ** を丁寧に読む（ここが根拠ゾーン）。
3. 依頼文の **目的の言い方（so we can / to / in order to）** を押さえ、 **動詞（助ける・決める・確認する等）** だけを核にする。
4. **these / it / that** があれば、 **直前の列挙や提案** に戻って「何について」電話するのかを確定する。
5. ほかの段落（見積もり・支払い・人員など）は、 **電話で求めている行為と一致するか** でYes/No判定して切る。
　ひっかかりやすい所： **注文書の項目** や **見積もり・支払いの話** に引っ張られて、肝心の「電話してほしい文」の目的を読まずに選ぶ。

選択肢のチェック（最後に）
✅ call依頼文の直前直後を読み、目的表現と指示語の参照先を特定してから選択肢をYes/Noで照合したか？`,
};
...
（D）2つ目の本文の①1～2行目で、値引きされた洗濯機を買おうと新店舗を訪れたことを述べているが、広告キャンペーンに対して感想を伝えているわけではない。feedback「意見、感想」、advertising「広告（の）」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「目的を聞く問題」** です。まずメールの **件名と書き出し** を見て、 **「何のために書いたか」** を判断します。
文書の種類が **「メール」** のときは、用件は **「苦情／称賛」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問がWhy型だと確認し、選択肢を **苦情／称賛／問い合わせ／広告への意見** の4つに分けて考える。
2. まず **メール** を優先し、 **件名→冒頭→末尾** の順で「用件がはっきり書いてある一文」を探して注目する。
3. **末尾** の定型表現（満足・感謝・称賛など）で、 **苦情か称賛か** を二択で判断します。
4. 中盤で **誰の行動をどう評価しているか** を押さえ、称賛の相手（人物など）が選択肢と合うか確認する。
5. ひっかかりやすい所： **途中の不満っぽい話（在庫が少ない等）だけで苦情と決める／「credit」を見てカード紛失の話に広げる** 。→この2つは切って、 **件名と末尾の結論** に範囲を戻す。

選択肢のチェック（最後に）
✅ 件名・末尾の確認→称賛対象の特定→選択肢類型への一致、の順で目的を確定したか（Yes/No）？`,
};
...
（A）（B）（C）1つ目の本文の②より、いずれもGrove家電店の店舗がある場所だが、新店舗だとの記述はない。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「複数の文書を見比べる問題」** です。まず設問のWhereを見て、 **「どの文書のどの情報を対応させるか」** を判断します。
文書の種類が **「広告／メール」** のときは、用件は **「案内／称賛」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「人物が会った場所（Where）」だと確認し、 **接客・会話が起きた場面の所在地** を探す方針にする。
2. メールで「会った場面」の文を押さえ、同じ段落から **場所の手掛かり** （訪問先／店の特徴語）を拾う。
3. メールに地名が出ないなら、広告の **店舗の並び** へ移り、メールの特徴語（例：新しい・最新）と **一致する候補** を探す。
4. 広告で特徴語に合う店舗が見つかったら、その **地名だけ** にしぼって選択肢と照合する。
5. ひっかかりやすい所：広告の地名が複数あると **「どこでも会える」** と考えてしまう。→Whereでも、 **新しい／唯一／最寄り** などの限定があれば、そこ以外は切る。

選択肢のチェック（最後に）
✅ メールの場所手掛かり（new/latest等）を広告の店舗情報に照合してから、地名を1つに絞ったか？（Yes/No）`,
};
...
（D）2つ目の本文の①3行目に、販売員のDawsonさんが店舗で同社製品の動画を見せてくれたことが述べられているのみ。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「いちばんありそうなことを選ぶ問題」** です。まず設問の *most likely* を見て、 **断定ではなく「条件からほぼそうなる」** を判断します。
文書の種類が **「広告／メール」** のときは、用件は **「やり取り／称賛」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「書いてある事実」ではなく **推論** かを確認し、 **“必然に近い推論”だけ** を狙う。
2. メール中盤〜後半から、 **買いたい物→制度に申請→承認** の流れを押さえて、行動のつながりを理解する。
3. 広告の支払い条件で、制度を使うための **必須条件（例：頭金＋分割）** を確認する。
4. 「承認された」事実と「必須条件」を **1対1で対応** させ、 **承認＝条件を満たした可能性が高い** と言えるか確認して選ぶ。
5. ひっかかりやすい所： **承認＝購入完了** と飛躍して、本文にない未来の行動まで選ばない／ **店で見た動画** を **家で見る予定** にすり替えない（根拠がなければ切る）。

選択肢のチェック（最後に）
✅ 推論の根拠をメール単体で完結させず、広告の条件文と1対1で対応づけてから選んだか（Yes/No）`,
};
...
（D）pose a risk to～「～に対して危険をもたらす」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「範囲を確かめる問題」** です。まず設問で対象（どれについて）を見て、 **「どの段落だけ読めばいいか」** を判断します。
文書の種類が **「報告書」** のときは、用件は **「状況の報告／見積もり」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問で **対象（どの部分についてか）** と、聞かれ方が **suggest＝結論・示唆** だと確定する。
2. 設問の主語を決める：聞かれているのは「南の採掘坑」だけだと確認し、 **対比の合図（例：対照を示す言い方）** で話題が切り替わる所を探し、北の話は一度捨てる。
3. その段落から **数量（どれくらい続くか）** と **評価語（採算がよい/利益が出る等）** をセットで押さえ、設問の「示唆」を自分の言葉で言い換える。
4. 選択肢は **期間表現の言い換え（具体→「数年間」など）** と **評価語の一致（利益が出る）** で対応づける。
5. ひっかかりやすい所： **別の対象の段落** から拾ってしまう／ **年数の言い換え** を見落とす。→「対象の段落か」「期間＋採算の2点がそろうか」以外は捨てる。

選択肢のチェック（最後に）
✅ 対象段落を対比語などで特定し、期間情報と採算評価の2点セットで選択肢を判定したか（Yes/No）`,
};
...
（C） Springbokコンクリート供給合同会社の社名は①の「依頼者」欄にあるので、この2人が同社のオーナーだとは考えられない。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「いちばんありそうなことを選ぶ問題」** です。まず **文書ヘッダー（発行元/会社名の表示）** を見て、 **「誰が書いた文書か」** を判断します。
文書の種類が **「報告書」** のときは、用件は **「調査結果の共有／今後の提案」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問の形を見て、「肩書きが書かれていないので、文書内の役割表示から判断する問題」と確認し、 **見る場所のうちヘッダー/Client/署名欄を先に見る** と決める。
2. **文書ヘッダー（発行元/会社名の表示）** で、「この報告書を出している組織」を押さえておく。
3. **冒頭のClient欄（依頼者＝誰かの切り分け）** を見て、発行元とは別の立場（依頼した側）を分ける。
   - ひっかかりやすい所： **Client欄を発行元と混同して依頼者側（所有者/顧客）と取り違える／本文が採掘の話なので現場の人だと早とちり** する。→ 依頼者情報は「誰のため」なので、人物の所属決めには最後まで使いすぎない。
4. **末尾のAssessment prepared by（作成者の役割ラベル）** と **作成者名の直前直後** を見て、「作成者＝報告書を作った側」かを確認する。
5. 残った選択肢のうち、 **発行元＋prepared by** と矛盾しない所属だけを選んで確定する（本文の作業内容から連想しない）。

選択肢のチェック（最後に）
✅ 発行元（ヘッダー）と依頼者（Client）を分けた上で、prepared by欄に基づいて所属を決めたか？`,
};
...
const tr02Question3 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
これは **「文を入れる問題」** です。まず前後の文を見て、 **「何のために入れる文か（追加の根拠／具体化／対比／結論）」** を判断します。
文書の種類が **「報告書」** のときは、用件は **「調査結果の共有／提案（推奨）の提示」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 挿入文の「役割」を決める：評価の言葉（低品質）＋指示語（残っている）→直前に「残っている砂利」の話がある場所を探す。
2. 挿入文の指し先を探す： **the gravel / that remains** が指すのは「直前で話している“残り”」なので、直前に **量の評価（very little など）** がある場所を探して注目する。
3. 各候補位置で、 **直前→挿入→直後** が同じ話題のままつながるかをYes/Noで判断する（特に **評価→結論** の流れ）。
4. **therefore など結論の合図の直前** に入ると、「量が少ない＋質も悪い」→「だからやめる」のように根拠が積み上がるかを確認する。
5. ひっかかりやすい所： **gravel という語がある場所** だけで選んで、 **In contrast など対比の境目** に入れてしまい、別の話題に誤接続する。対比の前後は「話題が切り替わる場所」なので基本は避ける。

選択肢のチェック（最後に）
✅ 挿入文の指示語（the gravel/that remains）の参照先と、therefore等の結論マーカー直前の根拠配置を両方確認したか（Yes/No）？`,
};
...
（D）①の1行目にMoriさんがカナダの大学を卒業したとあるが、創業者の出身地についての言及はない。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「範囲を確かめる問題」** です。まず設問の *indicated（示されている）* を見て、 **本文に書いてある事実の範囲** で判断します。
文書の種類が **「Webページ」** のときは、用件は **「経歴の説明／実績の紹介」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問を見て、 **「indicated＝本文に明示の事実（または直接言える範囲）」** だけを選ぶ、と決める。
2. **本文末尾** を先に見て、 **現在の状況・事業範囲のまとめ** を探す（会社紹介はここに集まりやすい）。
3. **地域名＋販売/売上/成長** のセットを目で探して、 **1つの地域に限られていない** ことを確認する。
4. 選択肢のキーワード（multiple/only/discount/from）を本文の言い方に合わせる。 **本文にない要素（割引・出身地など）は切る** 。
   - ひっかかりやすい所： **「大学の場所」「会議の場所」** を、 **販売先・展開先** と混同して選ばない。
5. 最後に、 **製品ラインの例** が出ていても、 **only（限定）** がないなら範囲を勝手に狭めない。
   - **強い限定語** （「〜だけ」「両方とも」「よく」など）は本文に同じ強さの言い方がないと外す。

選択肢のチェック（最後に）
✅ 地域名＋販売/売上を示す表現を本文から直接確認し、推測（割引・出身地など）で選んでいないか？(Yes/No)`,
};
...
const tr02An1Q2 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
これは **「文を入れる問題」** です。まず挿入文の **主語（代名詞）・時制・因果（discover など）** を見て、 **「前後に何が必要か」** を判断します。
文書の種類が **「Webページ」** のときは、用件は **「出会い→設立の流れ／経緯の説明」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問を見て挿入問題だと確認し、挿入文の **接続条件** を拾う（今回は **they / discovered / shared** ＝「複数人物」「会話の結果っぽい」）。
2. 各挿入位置の **直前** を見て、 **they の相手が複数人物として一つに決まるか** を Yes/No で判定する。
3. 次に **直後** を見て、流れが **行動・会話 → 気づき → 次の展開** になっているかを確認する。
4. **ひっかかりやすい所：** they を「組織」や「一般の人」まで広げたり、 **会話より前** に discovered を置いて因果を逆にする位置は切る。
5. 最後に、挿入文が直後の展開（交流が続く／共同で進む）への **理由** になっている位置を選ぶ（根拠は最大1文でOK）。

選択肢のチェック（最後に）
✅ 挿入候補を選ぶ前に、各位置で「theyの参照先が直前で確定するか」を全てYes/Noで確認したか？`,
};
...
（D） no longer「もはや～ない」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「目的を聞く問題」** です。まず発表文やコメントを見て、 **「何のために書いたか」** を判断します。
文書の種類が **「記事」** のときは、用件は **「変更のお知らせ／新しい方針の説明」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問の型を確認する：why＝出来事（名称変更）の **目的・意図** を本文から抜く、と決める。
2. 本文は **冒頭の発表** → **社長コメント周辺** に見る場所を決め、理由のキー（to / so that / we want / in order to / because）を探して注目する。
3. 見つけた理由文は、細部に寄せずに **「どう思われたいか／何を反映したいか（イメージ・位置づけ）」** の形に言い換えて押さえておく。
4. 選択肢は“目的一致”で照合する：その言い換えと **同じ方向** かをYes/Noで判断する。
5. ひっかかりやすい所： **名称変更＝覚えやすさ** などの一般論に飛んで、本文の we want/to を見ずに選ぶ。／ **「追加・拡大」→「もう売っていない」** のように極端に言い換えた選択肢は、本文にその否定がないので切る。

選択肢のチェック（最後に）
✅ whyに対して、本文の目的表現（to/we want/so that）を根拠として1文で言える状態で選んだか？(Yes/No)`,
};
...
（C） audience「視聴者、観客」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「目的を聞く問題」** です。まず設問の「何を成し遂げたいか（hope to accomplish）」を見て、 **「何のためにその行動をしたか」** を判断します。
文書の種類が **「記事」** のときは、用件は **「変更の連絡／宣伝」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問で「行動＝採用・起用」と「聞かれていること＝狙い」を押さえ、本文では **起用の狙いが書かれている場所** を探すと決める。
2. 本文で **hire / spokesperson / announced** など、**起用を述べる文**に直行し、同じ文の **to help / in order to** などの目的表現を拾う。
3. 目的表現の近くに **that / this** などがあれば、 **直前の文** に戻って「それ」が指す相手（不足している層・課題）を特定する。
   - ひっかかりやすい所： *参照先を取らずに** 「もともとの客層」っぽい方を選んでしまう。→ 指示語は必ず直前で回収し、思い込みは捨てる。
4. 特定した相手（誰を増やしたいか）を、選択肢の「誰が増える」に対応づける。
5. 本文にない **飛躍** （番組の視聴者が増える／店でイベントをする等）は、目的として書かれていないので外す。

選択肢のチェック（最後に）
✅ 起用を述べる文の目的表現と、指示語の参照先を特定した上で選択肢を選んだか（Yes/No）？`,
};
...
be set to do「〜する用意ができている」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「文を入れる問題」** です。まず挿入文の **主語・話題（何の話か）・いつ（予定/条件）** を見て、 **前後のつながり** で判断します。
文書の種類が **「案内」** のときは、用件は **「変更の連絡／新しい取り組みの説明」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 挿入文を分けて考える： **話題（ある人物を使った広告）／言いたいこと（開始時期）／いつ（名称変更のとき）** 。
2. 本文でその **人物名・話題語が初めて出る場所** を探し、それより前の候補は外す。
　ひっかかりやすい所： **人物名がまだ出ていない位置** に入れてしまい、何を指すか分からなくなる。→初出より前は切る。
3. 挿入文の **時間条件（〜のとき開始）** を受け止められる文（変更・開始・実施の話）が前後にある位置を探す。
4. 各候補で、前文→挿入文→後文が **同じ話題（人物/広告/施策）** の流れになり、挿入文が **開始時期の補足** として自然かを比べて決める。

選択肢のチェック（最後に）
✅ 挿入文の固有名詞が直前までに導入されており、前後1〜2文と同一トピックの流れを壊していないか（Yes/No）？`,
};
...
（D） expand「拡大する」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「いちばんありそうなことを選ぶ問題」** です。まず案内の冒頭と結びを見て、 **「全体として何が言えるか」** を判断します。
文書の種類が **「案内」** のときは、用件は **「方針の説明／お知らせ」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「suggested/分かる」かを確認し、 **書いてある事実→自然な言い換え（一般化）** を選ぶ方針にする。
2. 冒頭の1〜2文で、会社が強調していることを短くまとめる（例：対応の速さ・お客さま重視）。
3. 中盤は見出しごとに見て、共通点（保証・遅れたら補償など）を拾い、②のまとめを強める材料として束ねる。
   - ひっかかりやすい所： **時間や金額などの細部** に引っ張られて、設問が求める「全体の示唆」から外れる → 細部は「方針の裏付け」以上は追わない。
4. **末尾** で「目標・方針の明示」を拾い、冒頭と同じ方向か確かめる。
5. 選択肢を「本文に根拠がある一般化」か「本文にない新情報の持ち込み」かで分け、前者だけ残して、冒頭/末尾の方針と矛盾しないものに決める。

選択肢のチェック（最後に）
✅ 冒頭と末尾の方針表現を根拠にして、選択肢が「本文にない新情報」ではないとYes/Noで確認したか？`,
};
...
typically「通例、一般的には」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「文を入れる問題」** です。まず挿入文の主題語（online request／response time）を見て、 **「前後の文とどうつながるか」** を判断します。
文書の種類が **「案内」** のときは、用件は **「手続きの説明／保証の説明」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 挿入文の役割を決める： **オンラインで依頼した後の「回答までの目安時間」** を足す文だと押さえておく。
2. 各候補位置の **直前** を見る：そこが「オンラインでの依頼」を出していて、挿入文を置く前提ができている所だけ残す。
3. 同じ候補位置の **直後** を見る：挿入文のあとに **回答時間・遅れ・追加条件** の話が続く所だけ残す。
4. 見出し（例：請求関連など）をまたがない：挿入文の話題がその段落の範囲に収まる位置を選ぶ。
　ひっかかりやすい所： **「時間」が出るからといって別の段落の時間枠（予約や停止など）に結び付けてしまう** 。→主題語（online request）が出ない段落は切る。

選択肢のチェック（最後に）
✅ 挿入位置の直前で online request の前提が作られ、直後が回答時間/遅延の説明として読めるかをYes/Noで確認したか？`,
};
...
（D）②に、Leffleyさんは面接中に今週国外にいる予定だと言ったとあるが、面接の予定を変更したとは述べられていない。reschedule「～の予定を変更する」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「範囲を確かめる問題」** です。まず人物についての **明示情報の範囲** を見て、 **「何が本文で言えるか」** を判断します。
文書の種類が **「チャット」** のときは、用件は **「変更の連絡／確認のやり取り」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「示されている」かを確認し、 **書いてある事実（＋必然的に言えること）だけ** で人物の特徴を1つ決める方針にする。
2. 冒頭で、話題の枠を押さえる（ **対象は新しく入る側か／欠席者がいるか** ）。ここで人物の立ち位置（新規・既存・外部）を決める。
3. 人物名が出たら、その文だけで終わらせず、 **直前の一般的な言い方（欠席者・新しく入る人など）と結び付けて** 「この人＝どの役割か」を確定する。
   - ひっかかりやすい所： **名前の文だけ見て** 、役割の結び付けをせずに「外部の人／既存の人」と早とちりする。→必ず直前の役割語に戻って確認する。
4. 選択肢は「本文に明示されている／その役割なら必ず言える」だけを残す。 **本文にない行動** （会議に出る、面接を変える等）や、 **本文と反対の形容** （経験がある等）は切る。
5. 最後に、残った主張が「その人が研修の対象として扱われている」ことと矛盾しないかを見て1つに決める（根拠は本文の一か所で足りる）。

選択肢のチェック（最後に）
✅ 人物名が出た文を、直前の一般名詞（役割）に結び付けてから選択肢判定したか（Yes/No）？`,
};
...
（A）①で、指導係は明日の研修会に来る予定だと言っている。currently「現在、目下」、unavailable「都合がつかない」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「いちばんありそうなことを選ぶ問題」** です。まず発言の前後を見て、 **「何のために言ったか」** を判断します。
文書の種類が **「チャット」** のときは、用件は **「予定の調整／対応策の提案」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問の言い方を確認する： **most likely** なら、言葉どおりより **意図（提案・推測）** を取りに行く。また設問の時刻の発言だけを中心に見る場所を決め、直前の発言を「原因」、直後の発言を「評価・修正」として読む。
2. 引用文の直前を見る： **誰が何に参加できないのか／理由は何か** を押さえておく。
3. 引用文を言いかえる：つなぎ言葉（Then など）に注目し、「何を受けての提案か」を直前の確定情報にしぼる。 **「来週また」＝日程を後ろにずらす提案** 。ずらす対象（研修・訪問など）と、狙い（欠席者への配慮）。
4. 引用文の直後で確かめる：相手が **否定＋代替案** を出すなら、引用文は「欠席者のためにやり直す」方向の提案だったと判断する。
5. 選択肢は「誰の予定の話か」「週の情報が合うか」で合わせる。 **ひっかかりやすい所：** 不在なのが **指導係** だと取り違える／週の情報を落として本文にない理由（資料不足・休暇）を選ぶ → 不在なのは誰か、いつ来る予定かを本文で確認して切る。

選択肢のチェック（最後に）
✅ 引用文の直前直後を根拠に、提案の対象（何を来週にするか）と目的（誰のためか）を言えますか（Yes/No）`,
};
...
（D）報告書の修正については話題に上っていない。revise「～を修正する」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「いちばんありそうなことを選ぶ問題」** です。まず引用句の前後を見て、 **「何がどう変わったのか」** を判断します。
文書の種類が **「チャット」** のときは、用件は **「変更の連絡／確認のやり取り」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「most likely mean」なので、引用句だけで決めず、 **引用句の直後** で意味を確定すると決める。
2. 引用句の直後1〜2文を読み、 **update＝何の変更か** を「期限／リンク／価格／修正可否」みたいに **具体名詞1つ** に落とす。
3. **引用句の直前** で共有されていた前提（特に期限など）を拾い、直後の新情報と比べて**差分**があるかを見る。
4. *today / due date* などの **時間表現** が出たら、旧条件→新条件の順に並べて、変更点を1つにしぼる。
　ひっかかりやすい所：updateを直前の別話題（例：リンク）に連想で飛ばし、 **直後の具体説明** を読まずに選ぶ。→「updateの中身は直後に書かれる」と決めて、そこ以外はいったん切る。
5. 選択肢は「その変更点を一文で言い換えたものか」をYes/Noで照合し、いちばん一致するものを選ぶ（根拠は最大1つで十分）。

選択肢のチェック（最後に）
✅ 引用句の直後の文から「何が変わったか」を1つに特定してから選択肢を照合したか（Yes/No）？`,
};
...
（D）①で、アップロードができないというトラブルへの言及があるのみ。technical「技術上の」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「指すものを選ぶ問題」** です。まず **人名の周りの“役割を表す言葉”** を見て、 **「何者として出ているか」** を判断します。
文書の種類が **「チャット」** のときは、用件は **「変更の連絡／確認のやり取り」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が **most likely（推定）** かを確認し、 **固有名詞そのものではなく“役割語”に置き換えて考える** と決める。
2. 人名が出る発言を探し、直前直後の **依頼・指示の言い方（requested など）** を押さえて、上下関係の手掛かりを集める。
3. **指示語（that ～ など）** が出たら、必ず「何を指すか」を直前の発言に戻って確定する（同じ連絡を別の言い方で言っていないかも確認）。
4. 確定した内容を **役割語（例：上司など）** に置き換え、選択肢の立場と一致するものを選ぶ。
　ひっかかりやすい所： **話題語（作業・不具合）から職種を連想して飛ぶ** 。→本文で役割が言われていないものは切る。

選択肢のチェック（最後に）
✅ Mr. Wood を「同一の連絡/依頼」を手掛かりに役割語へ置換してから選択肢判定したか（Yes/No）？`,
};
...
（D）④で、資材の見積もりが未完了だと述べているだけ。be unsure about〜「～について確信がない」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「範囲を確かめる問題」** です。まず設問の主語を見て、 **「何について心配しているか」** を判断します。
文書の種類が **「チャット」** のときは、用件は **「変更の連絡／確認のやり取り」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問で **「誰の懸念か」** を確定し、見る範囲を **その人の発言だけ** に決める（他の人の発言は背景）。
2. その人の発言から、 **心配・未完了・必要** の合図（still / need to / haven’t / can’t など）を探して注目する。
3. 見つけた内容を、具体名を外して **「未完了の作業／できていない状態」** のように言い換える。
4. その言い換えと、選択肢の **動詞・状態語** （未完了／不参加／誤操作／不確実）を1対1で合わせる。
5. **ひっかかりやすい所：** 「見ていない」から「誤って削除した」など **本文にない行動を足す** ／予定の話を「出席できない」に **広げる** 。→ **明示がないものは切る** 。

選択肢のチェック（最後に）
✅ 懸念の根拠を、該当人物の発言中の未完了・必要表現に限定してYes/No判定したか？`,
};
...
（D）Dengさんは⑤で、早めに念を押さなかったことを謝っているが、TillmanさんがDengさんに確認や念押しを目的としたリマインドメールを送るとは述べていない。reminder「思い出させるもの」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「指すものを選ぶ問題」** です。まず下線部の前後を見て、指示語（it）が **何を指すか** を判断します。
文書の種類が **「チャット」** のときは、用件は **「変更の連絡／確認のやり取り」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問の形を確認： **most likely＋慣用表現** なので、直訳よりも「何を引き受けた発言か」を探すと決める。
2. 引用部の直前へ戻り、そこで出た **“やるべき作業”を1つ** 拾う（it候補はここにある）。
3. it をその作業に置き換えて読んで、 **「私がそれをやる」** という流れになるか確認する。
4. 選択肢は **(a)作業が一致 (b)本文に話題がある** の2点でYes/No判定する。
　ひっかかりやすい所： **会話冒頭の状況（閉館・戸締まり）** に引っ張られて、it を別の行動に結びつけない。状況説明は切って、 **直前の作業** を優先する。
5. 最後に、 **誰がその作業をする流れか** （引き受けているのはどちらか）を直前直後で確認する。

選択肢のチェック（最後に）
✅ 引用部のitの参照先を直前のタスクに結びつけてから選択肢をYes/No判定したか？`,
};
...
（D）⑦より、TillmanさんがDengさんの頼み事を聞いたことが分かる。それに対してDengさんはお返しをしたいと発言しているのみ。do～a favor「〜の願いを聞く、～に便宜を図る」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「いちばんありそうなことを選ぶ問題」** です。まず設問の *suggested* を見て、 **「発言から自然に言える人物情報」** を判断します。
文書の種類が **「チャット」** のときは、用件は **「変更の連絡／感謝」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「suggested」かを確認し、明示ではなく **推測** だと押さえる（人物の勤務時間・役割・関係を探す）。
2. 設問の主語を確認し、「その人について分かること」だけを探す（相手側の情報は混ぜない）。
3. 会話の **末尾** を読み、感謝・埋め合わせ・交代などの **事実** を拾う。
4. 事実の中の **時刻表現** と、 **regular/usually/daily** の近くを見て、「その人の普段の行動」に結びつく形に言い換える（時刻→勤務開始の時間帯）。
5. 選択肢は「人物について言えること」になっているか確認し、本文の事実から **一段の推測** で届くものだけ残す。 **ひっかかりやすい所：** 開館時刻の話を読んで、担当者まで勝手に決めない（時刻＝担当者と決めつけない）。また「お返ししたい」は未来の意向なので、過去に相手へ何かした事実と混ぜない。

選択肢のチェック（最後に）
✅ 人物に関する結論を、末尾のシフト言及＋時刻/頻度語の根拠に戻してYes/Noで説明できるか？`,
};
...
（D）⑨で、資格があればできる作業としてソース作りが挙げられているのみ。server「給仕人」。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「いちばんありそうなことを選ぶ問題」** です。まず **「明日」** を見て、 **「明日起こる確度が高い出来事」** を判断します。
文書の種類が **「チャット」** のときは、用件は **「予定の調整／依頼」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問の条件を先にそろえる： **most likely＋tomorrow＝「明日の確定に近い予定」** を1つ探す。
2. 本文で **「明日」** に触れている発言へ戻る（start tomorrow / tomorrow などの時間の手がかり）。
3. その出来事が **依頼→承諾** 、または **開始日が確定** している形かを確認して、確定イベントとして扱う。
4. ひっかかりやすい所： **「〜できるようになる」「必要なとき」** のような条件付きの話を、 **明日やる予定** と取り違える。条件が付く話は切る。
5. 最後に、残った候補が **本当に「明日」** に当てはまるかだけを見直して決める（別日の予定は外す）。

選択肢のチェック（最後に）
✅ 「明日」と明示された箇所に戻り、依頼→承諾（または開始確定）の形で裏取りしてから選んだか？(Yes/No)`,
};
...
（D）直前のPlebaniさんの発言で、資格があればできる作業として生地作りに言及があるだけで、Berkさんがこの時点で生地を作る人を探すことを意図しているとは考えられない。`,
  metacogFeedbackJa: `このタイプを解くには
これは **「いちばんありそうなことを選ぶ問題」** です。まず引用発言を単独で訳さず、 **直前直後の流れ** を見て、 **「その発言が何への返事か」** を判断します。
文書の種類が **「チャット」** のときは、用件は **「依頼／予定の調整」** になりやすい。だから最初にどちらかを予想し、最後に本文で決定する。

解答への手順

1. 設問が「most likely mean」だと確認し、発言の **意図（断り/提案/確認など）** を取りに行くと決める。
2. 引用発言の直前で、 **誰が誰に何をしてほしいか＋いつか** （行為＋曜日）を押さえる。
3. 引用発言を「直前の依頼への返事」として読み、 **できる／できない** のどちらを伝えているか判断する。
4. 引用発言の直後で、相手が **忘れていた／代わりにやる** のように動くかを見て、解釈を確定する。
   - ひっかかりやすい所： **「day off」を一般的な休暇の話に広げて** 、直前の依頼への返事だと捉え損ねる。→「この曜日のこの役割は無理」という話だけ残し、余計な想像はやめる。
5. 選択肢は、 **行為の種類・曜日・可否（できない）** までそろうものだけを選ぶ。

選択肢のチェック（最後に）
✅ 引用発言の直前の依頼内容と直後の反応を確認してから、可否（できない）まで一致する選択肢を選んだか？`,
};
