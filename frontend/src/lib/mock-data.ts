...
**これは「目的を聞く問題」です。まず手紙の宛先・形式を見て、「何のために書いたか」を判断します。**
文書の種類が「手紙」のときは、用件は「感謝／苦情」になりやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. 設問が「なぜ書いたか」なら、見る場所を決める：広告は背景、答えは「手紙本文」から取る。
2. 手紙の結びを先に読む：最終段落→その直前の段落で、感謝している／困っている／お願いしている などの言葉を拾う。
3. 拾った言葉を「言いたいことの種類」にまとめる（感謝・称賛／不満・改善してほしい／訂正してほしい など）。
4. ひっかかりやすい所：冒頭のトラブルだけで「懸念・苦情」と早とちりする → 冒頭は“きっかけ”として置き、後半に要求や不満があるかを確認。なければ苦情系は捨てる。
5. 選択肢は動詞で合わせる：称賛する／懸念を表す／招く／誤りを報告する のどれが、結びの言葉と同じかだけ残す。

選択肢のチェック（最後に）
✅ 「感謝・称賛＋今後も利用する」など前向きな結びになっている（Yes/No）`,
};
...
（C） be unable to do「〜することができない」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「複数の文書を見比べる問題」です。まず手紙と広告のどちらに“行動”と“条件（料金など）”が書かれているかを見て、つなげて判断します。**
**文書の種類を見る** → 手紙は「何をしたか」、広告は「その条件（料金・日数）」が出やすい。

解答への手順

1. 設問が人物についての「示唆」なので、**片方だけでは出ないが、2つを合わせると確定する事実**を探すと決める。
...
文書の種類が「取引先へのメール」のときは、用件は「遅れ連絡／変更のお知らせ」になりやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. **件名＋冒頭1〜2文**だけを見て、メールの用件を短く作る（例：注文品の配送状況の連絡）。
2. 本文から“変化/問題”の合図（最短・予定より遅い・中止・残念ながら等）を拾い、**何が起きたか**を決める。
3. **目的がはっきり書いてある一文**（予定より遅れる、など）で、用件を確定する。
4. 選択肢は“主目的”で選ぶ。**ひっかかりやすい所：**補償や値引きの話が出ても、それが「おまけ」なら主目的にしない（主題が最初からそれかどうかで捨てる）。
5. ほかの選択肢は範囲で外す：依頼・方針説明・割引提供が**冒頭から主題として言われているか**を確認し、なければ除外する（表は選択肢に必要なときだけ見る）。

選択肢のチェック（最後に）
✅ 「このメールを短い件名に言い換えると“納期遅延のお知らせ”になる」→ Yes / No`,
};
...
const tr01An1Q2 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
**これは「複数の文書を見比べる問題」です。まずメールから“表に当てられる手がかり（日時など）”を1つ見て、「どこに届く話か」を判断します。**
**文書の種類を見る** → メール＋スケジュール表なら「遅れ連絡／日程確認」になりやすいので、まず日付を拾って表で確かめます。

解答への手順

1. 設問の「目的地」を**最終到着地だと決めつけず**、まずメールで“到着日などの手がかり”を1つ確定する（ここでは到着日）。
2. メール内の会社名・対象物から、**どの表を見るべきか**を決める（この海運会社の表だ、と判断）。
3. 表でその日付を探して**一致する行**を特定する。
4. 一致した日付が入っている**列見出し（港＋国）**を読み、国名に直して選ぶ。
5. ひっかかりやすい所：**便の終点の国を目的地だと早とちり**／**近い日付**を拾う。→必ず「メールの手がかり（日付）」と表のセルを一致させ、そこから列見出しを取る。

選択肢のチェック（最後に）
✅ 自分の選んだ国は「メールの手がかりと同じ日付が書かれているセルの列見出し」から取ったと言えますか？（Yes/No）`,
};
...
const tr01An1Q3 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
**これは「表を使う問題」です。まず設問の地名と条件を、表の「どの列を見るか」に言いかえて判断します。**
**文書の種類を見る** → 今回はスケジュール表なので「出発地の列／到着地の列／途中の列」を先に決めます。

解答への手順

1. 設問の条件を表の列に翻訳する：出発地＝カナダの列、到着地＝日本の列、「直行」＝間の列が空欄（—/記載なし）であること。
2. まず出発地の列に日付がある行だけ残す（出発していない行は捨てる）。
3. 残った行で、出発地と到着地の間にある中間の列を左右に見て、両方とも空欄の行だけ残す。
   - ひっかかりやすい所：「cancelled」を「寄港しない＝直行」と思い込む。→ 直行の判定は“空欄かどうか”で行い、「cancelled」は別扱いで見る。
4. 最後に到着地の列に日付があることを確認し、行の船名を選択肢と照合して選ぶ。

選択肢のチェック（最後に）
✅ 選んだ行で、出発地→到着地の間の中間列が「両方とも空欄」になっているのを確認したか？（Yes/No）`,
};
...
（D）1つ目の本文の②より、イベントの予定終了時刻。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「複数の文書を見比べる問題」です。まず“誰が言っているか”を見て、どの文書の情報を基準にするかを判断します。**
文書の種類が「申込書＋返信メール」のときは、申込書に元の予定、メールに変更提案が書かれやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. **指すものを選ぶ**：設問の人物が本文の誰かを特定する（メールの送信者＝設問の人物）。
2. **見比べる**：メールで「開始時刻」に関する提案を探し、内容をそのまま抜き出す（例：何時間“早く/遅く”）。
3. **数・時間を計算する**：申込書のイベント時間から「開始」を基準に取り、提案どおりに±時間して新しい開始時刻を出す。
4. **選択肢に当てる**：計算した時刻と一致するものだけ残す。
　ひっかかりやすい所：**終了時刻**を基準にして計算してしまう／申込書の開始時刻を**そのまま**選んで提案（早く）を反映しない → 「開始をどうする話か？」だけに絞って他は捨てる。

選択肢のチェック（最後に）
✅ 「早く/遅く」を**開始時刻**に当てて計算しましたか（Yes/No）？`,
};
...
（D）1つ目の本文の注文書の⑧の「スタッフが必要」の項目の「はい」にチェックがあるが、server「給仕人」の数の確認は求めていない。clarify「〜を明らかにする」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「目的を聞く問題」です。まず依頼の言い方（電話して、など）を見て、「何のためにしてほしいか」を判断します。**
**文書の種類を見る** → 予約に関するメールは「確認／提案」が多いので、そのどちらかを予想し、本文で決める。

解答への手順

1. 設問が「なぜ〜するように求めたか」なので、メール内の「電話して」に当たる依頼表現を探す（例：電話して、連絡して）。
2. その直後の目的の形（〜するために／〜できるように）を取り、電話の目的を短い言葉にする。
3. 目的の中に「これら」などがあれば、直前の列挙に戻って「何についての話か」を具体化する（何を決めるのか、まで言い切る）。
4. 選択肢は「電話の目的」と一致するものだけ残す。**ひっかかりやすい所：**見積もりや支払いの段落が目立つが、「電話の目的」として書かれていないなら捨てる／別の情報（スタッフなど）も「確認してほしい」と明示がなければ捨てる。

選択肢のチェック（最後に）
✅ 「電話して」の直後の目的を読み、さらに「これら＝何の決定か」まで本文の直前で特定できた（Yes/No）？`,
};
...
（D）2つ目の本文の①1～2行目で、値引きされた洗濯機を買おうと新店舗を訪れたことを述べているが、広告キャンペーンに対して感想を伝えているわけではない。feedback「意見、感想」、advertising「広告（の）」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「目的を聞く問題」です。まず件名と書き出しを見て、「何のために書いたか」を判断します。**
取引先へのメールは感謝や苦情が多い。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. 設問が「なぜ送ったか」なら、まず件名＋書き出し直後で「言いたいことの種類」をどれかにいったん決める（感謝／不満／お願い／問い合わせ など）。
2. 次に、本文の後半〜結びで「感謝している／満足している」などの言葉を探し、誰（店・担当者）に向けたものかも一緒に取る。
3. その人が何をしてくれたかの行動描写をざっと確認し、「称賛」なのか「苦情」なのかを二択で判断します。
4. 選択肢は「言いたいことの種類」で照合し、本文に書かれていない前提が必要なものは範囲外として落とす。**ひっかかりやすい所：**「credit」を“カード”と決めつけて紛失の話に飛ぶ／在庫がない＝苦情と早とちりする → 結びが感謝・満足で終わっているかを優先して確認する。

選択肢のチェック（最後に）
✅ 結びの文が「不満」ではなく「感謝・満足」になっている（Yes/No）？`,
};
...
（A）（B）（C）1つ目の本文の②より、いずれもGrove家電店の店舗がある場所だが、新店舗だとの記述はない。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「複数の文書を見比べる問題」です。まず設問の聞き方（どこで会ったか）を見て、答えが「場所」だと判断します。**
**文書の種類を見る** → 広告は「店舗情報」が出やすいので、場所の特定に使えると予想します。

解答への手順

1. 設問の焦点を確定する：「どこで会ったか」＝面会した**店舗の所在地**を答える、と見る場所を決める。
2. Eメールで面会シーンを探す：相手が登場する直前直後を見て、場所の手がかり（「新しい店舗」など）を抜き出す。
3. その手がかりが地名でないなら、広告へ移動する：「新しい／最新の店舗」に当たる表現がある行を探す。
4. 広告の「最新の店舗」＝どの都市かを1つに決め、選択肢の都市名に当てはめて選ぶ。
ひっかかりやすい所：店舗名がいくつも並ぶと、条件（新しい／最新）を無視して適当に選びがち。**「新しい」という条件に合わない都市は捨てる。**

選択肢のチェック（最後に）
✅ 「新しい店舗」という条件を、広告の「最新の店舗」に1対1で対応づけてから都市を選びましたか？（Yes/No）`,
};
...
（D）2つ目の本文の①3行目に、販売員のDawsonさんが店舗で同社製品の動画を見せてくれたことが述べられているのみ。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「いちばんありそうなことを選ぶ問題」です。まず設問の範囲（本文に書いてある事実＋そこからほぼ確実に言えること）を決めて、根拠が2つ以上そろうものを選びます。**
**文書の種類を見る** → 広告は条件が書かれやすい。だから「制度の条件」を拾って、メールの内容とつなげます。

解答への手順

1. 設問の言い方を確認し、「本文に書いてないが、本文からほぼ確実に言えること」を選ぶと決める。
2. 選択肢を「すでに起きた事実」か「これからの行動」かに分け、まず前者を優先して根拠を探す。
3. メールで“確定情報”を拾う：制度に「申請した」「承認された」など、事実として言い切っている所を押さえる。
4. 広告に戻り、制度の“必須条件”（頭金＋分割など）を確認して、メールの確定情報と対応づける。
5. 推論は最小限にして選ぶ。**ひっかかりやすい所：**メールだけで判断して広告の条件を見落とす／「動画を見た」→「家でも見る」など本文にない行動を“ありそう”で選んでしまう。根拠が書かれていない行動は捨てる。

選択肢のチェック（最後に）
✅ 選んだ内容は、本文の明示情報が少なくとも2つ以上そろって「Yes」と言えますか（Yes/No）?`,
};
...
（D）pose a risk to～「～に対して危険をもたらす」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「範囲を確かめる問題」です。まず設問の対象（どこについて？）を見て、「どの段落だけ読めばいいか」を判断します。**
**文書の種類を見る** → 報告書は「判定・見積もり（続けられる期間／採算）」が多いので、期間と利益の結論を探します。

解答への手順

1. 設問の主語を決める：聞かれているのは「南の採掘坑」だけだと確認し、北の話は一度捨てる。
2. 見出しの目的（何を判定する報告か）を見て、「継続できる期間」と「採算」が答えになりやすいと予想する。
3. 南の段落で結論を拾う：条件（今のペースなら）→期間（あと何年）→採算（利益が出るか）の順で線を引く。
4. 選択肢は言い換えで合わせる：数字の表現（例：あと数年）と、利益の表現（例：採算が取れる）を対応させる。
　ひっかかりやすい所：北の段落の「来年中に終了」などを、南の話と取り違える／「〜すべき」「追加で調べる」「危険」など強い言い方を、本文に書いていないのに想像で選ぶ → 本文にその指示語がなければ切る。

選択肢のチェック（最後に）
✅ 「あと何年続く」＋「利益が出る」の両方が本文にそろっている（Yes/No）`,
};
...
（C） Springbokコンクリート供給合同会社の社名は①の「依頼者」欄にあるので、この2人が同社のオーナーだとは考えられない。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「いちばんありそうなことを選ぶ問題」です。まず「誰が書いた文書か」を見て、人物の立場を判断します。**
**文書の種類を見る** → 報告書は「発行元／依頼者／作成者」が分かれて出やすいので、まずそこを見ます。

解答への手順

1. 設問の形を見て、「肩書きが書かれていないので、文書内の役割表示から判断する問題」と決める。
2. 末尾の署名・クレジット行（prepared by など）を探し、その人物が「作成者」だと確定する。
3. ヘッダーの発行元（会社名）と、依頼者欄（client など）を別物として分けて読む。
4. 「作成者」→「発行元側の人」とつなげて、選択肢の所属（どの会社の人か）に当てはめる。
5. ひっかかりやすい所：依頼者欄が先に出るので「依頼者＝作成者」と取り違える／本文が採掘の話なので現場の人だと早とちりする → 役割は本文の作業内容ではなく、prepared by の行で決めて、合わない選択肢は捨てる。

選択肢のチェック（最後に）
✅ 作成者名の直前の表現（prepared by / written by など）を見て、その人の役割を一言で言えますか？（Yes/No）`,
};
...
const tr02Question3 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
**これは「文を入れる問題」です。まず挿入文の指示語とつながり方を見て、「どんな役目の文か」を判断します。**
報告書のような文書は、調査→結果→結論（勧告）の流れが多い。だから「結果を強める追加情報」かどうかを予想し、前後の文で確かめる。

解答への手順

1. 挿入文の「役割」を決める：評価の言葉（低品質）＋指示語（残っている）→直前に「残っている砂利」の話がある場所を探す。
2. 各候補位置で前後1文ずつ読む：指示語が何を指すかが、直前でハッキリ出ているかを確認する。
3. 接続の流れを見る：挿入の後に結論へ進む合図（従って など）が来るなら、「悪い根拠を足して結論を強める」形になっているかで判定する。
4. 話題の切り替えの近くは避けて再確認する：対比の合図（対照的に など）の前後に入れると、北側/南側どちらの話かがズレやすい。
   - ひっかかりやすい所：評価の言葉（低品質）だけで選び、量が多く前向きな段落側に入れてしまう → 「残っている」がどの砂利か言い換えできないなら捨てる。

選択肢のチェック（最後に）
✅ 挿入後に「残っている砂利＝（どちら側の）わずかに残る砂利」と1つに決めて言い換えできますか？（Yes/No）`,
};
...
（D）①の1行目にMoriさんがカナダの大学を卒業したとあるが、創業者の出身地についての言及はない。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「範囲を確かめる問題」です。まず設問の対象（会社の何が“示されている”か）を見て、「本文に書いてある事実」を拾います。**
**文書の種類を見る** → 会社紹介のページは「今どこで事業をしているか／どこで売れているか」が最後にまとまりやすい。だから最後の現状まとめを先に見ます。

解答への手順

1. 設問を言い換える：「〜について何が示されている？」＝**本文に明示される事実**を選ぶ、と決める。
2. 見る場所を決める：会社の現状は末尾の「現在」を述べる部分に集まりやすいので、最後の数文を優先して読む。
3. 選択肢の核語を本文語に変換する：「複数の国」→ **地名が複数出る／複数地域での販売・売上**を示す表現、に置き換えて探す。
4. 一致を判断する：地名の並び＋販売/売上の動詞がそろえば採用する。
5. ひっかかりやすい所：**強い限定語**（「〜だけ」「両方とも」「よく」など）は本文に同じ強さの言い方がないと外す。価格情報や出身地の断定も、書いてなければ捨てる。

選択肢のチェック（最後に）
✅ 選んだ内容を、本文の「地名」＋「売上/販売を示す動きの言葉」を使って、同じ意味で言い直せますか（Yes/No）？`,
};
...
const tr02An1Q2 = {
  ...
  metacogFeedbackJa: `このタイプを解くには
**これは「文を入れる問題」です。まず挿入文の主語や動き（発見・結果）を見て、「前に何が必要か／後ろに何が来るか」を判断します。**
文書の種類が「会社紹介のウェブページ」のときは、用件は「経歴の流れ／出会い→設立の流れ」になりやすい。だから最初に流れを予想し、最後に本文で決める。

解答への手順

1. 挿入文の“入る条件”を抜き出す：主語が複数（they）／「発見した」は直前に会話や交流が必要、をメモする。
2. 各候補で前後1文ずつ見る場所を決める：直前に複数の人物がはっきり出ているか（単数→複数の切替も確認）。
3. 直前が「会話・やりとり（きっかけ）」になっているかを確認する（発見は突然出にくい）。
4. 直後が「その結果どうなったか（やりとりが続く／設立につながる）」になっている位置を優先する。
   - ひっかかりやすい所：単語が近い（スポーツウエア等）だけで選ぶと、**会話→発見→結果**の順番が崩れるので、その候補は捨てる。

選択肢のチェック（最後に）
✅ 挿入後、前後だけで「何がきっかけで分かった？分かった結果どう進んだ？」を説明できますか（Yes/No）`,
};
...
（D） no longer「もはや～ない」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「目的を聞く問題」です。まず設問の why を見て、「理由・ねらい」を言っている場所を探して判断します。**
文書の種類が「告知記事」のときは、用件は「変更のお知らせ／新しい方向性の説明」になりやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. 設問が why なら、本文の「発表の直後」と「引用コメント」を先に見に行く（理由が置かれやすい）。
2. 引用コメントで、(a)何が変わったか → (b)どう思ってほしいか、の2点を拾う。
3. (a)+(b)をまとめて「今の店の方向性」を短く言える形にする（例：提供内容が広がったので、そう見てほしい）。
4. 選択肢は、その方向性に合う“広い言い方”を残し、本文に書いていない話は切る。**ひっかかりやすい所：**「覚えやすいから」などのもっともらしい一般論は、本文に根拠がなければ捨てる／「追加された」だけなのに「もう売っていない」と極端にして選ばない。

選択肢のチェック（最後に）
✅ 自分の選択を、本文中の言い回しを使って言い換えて支えられますか（Yes/No）？`,
};
...
（C） audience「視聴者、観客」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「目的を聞く問題」です。まず設問の「何を成し遂げたいか」＋「何をして（採用して）？」を見て、本文の *to / help (to) / in order to* を探して判断します。**
文書の種類が「記事」のときは、用件は「発表／ねらいの説明」になりやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. 設問を分解する：「会社が望む結果」×「手段（採用することで）」にする。
2. 本文で *hire … to help …* などの“ねらいを言う形”を探して、そこを答えの中心にする。
3. その文に指示語があれば（that/this など）、**直前の文**に戻って「具体的に誰のことか」を決める。
4. 指示語の中身（客層など）を、選択肢の「対象（誰が増えるか）」と照合して一致するものだけ残す。
   - ひっかかりやすい所：指示語を戻らずに「若者向けの店だった」など別の話題に乗ってしまう／有名人の出演番組に引っぱられて「番組の利益」を会社のねらいだと勘違いする → **“会社が客を増やしたい相手”だけを見る**。

選択肢のチェック（最後に）
✅ 「指示語が指す客層」を直前文から言えて、その客層と選択肢の対象が一致していますか（Yes/No）?`,
};
...
be set to do「〜する用意ができている」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「文を入れる問題」です。まず挿入文の主語と時間の言い方を見て、前後の流れが自然になる場所を判断します。**
**文書の種類を見る** → 記事は「発表→理由→追加情報」の流れが多いので、補足文は“説明のあと”に来やすいです。

解答への手順

1. 挿入文を分けて考える：話題（ある人物を使った広告）／言いたいこと（開始時期）／合図（社名が変わるとき）。
2. 候補位置を上から見て、挿入文の人物がまだ出ていない場所は消す（固有名詞は基本「初めて説明したあと」に置く）。
3. 人物が出たあとの部分で、「広告の中身・ねらい」→「運用情報（いつ始まる）」の順になる場所を探す。
4. 時間のつながりを最後に確認する：「社名が変わるとき」という条件が、周りの社名変更の話と矛盾しないかを見る。
   - ひっかかりやすい所：「広告」という言葉だけで、最初の広告更新の話に入れたくなるが、その時点では人物が未登場なら捨てる／人物の初登場直後に入れると、人物説明より先に開始時期が来て順番が不自然になりやすい。

選択肢のチェック（最後に）
✅ 入れたあとに「その人物が誰かの説明がすでに終わっていて」かつ「広告の説明→開始時期」の順で読めますか（Yes/No）`,
};
...
（D） expand「拡大する」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「いちばんありそうなことを選ぶ問題」です。まず冒頭と結びを見て、「全体として何が言えるか」を判断します。**
**文書の種類を見る** → 会社の案内文は「お知らせ／お願い」よりも「方針の説明（サービスの約束）」が多い。だから最初に“会社の姿勢”を予想し、最後に本文で決める。

解答への手順

1. 設問が *suggested* かを確認し、「細かい数字合わせ」ではなく「本文全体から言えること」を探すモードに切り替える。
2. 冒頭の1〜2文で、会社が強調していることを短くまとめる（例：対応の速さ・お客さま重視）。
3. 中盤は見出しごとに見て、共通点（保証・遅れたら補償など）を拾い、②のまとめを強める材料として束ねる。
4. 選択肢は Yes/No で切る：本文の複数の場所（冒頭＋結び＋具体例）で同じ方向ならYes、本文にない話題ならNo。
   - ひっかかりやすい所：数字（時間や金額）に引っ張られて「数字が合う選択肢」を探し始めること。→ 今回は数字は“姿勢の裏づけ”なので、数字そのものを言っていない選択肢は捨てる。

選択肢のチェック（最後に）
✅ 選んだ内容を、冒頭と結びの両方で言い換えて支えられますか？（Yes/No）`,
};
...
typically「通例、一般的には」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「文を入れる問題」です。まず挿入文の話題（オンラインでの依頼→返信までの時間）を見て、「どの話の流れに足す文か」を判断します。**
**文書の種類を見る** → 案内文なら「手続きの方法／時間の目安」が多いので、その並びになる場所を探します。

解答への手順

1. 挿入文のキーワードを抜く（online request／response／within two hours）→「手段（オンライン）×返信時間（2時間）」の補足だと決める。
2. 各候補位置の直前直後を見て、直前に“オンラインでの問い合わせ”が出ているかを探す（portal など）。
3. 「手段を示す文」→「その手段の所要時間」→「例外や追加条件（さらに調査なら数日）」の順で自然につながる場所を選ぶ。
4. ひっかかりやすい所：**「2時間」という数字だけ**で、訪問の時間枠の話に入れない。数字一致より「online/request/response」の話題一致を優先して、話がズレる候補は捨てる。

選択肢のチェック（最後に）
✅ 挿入後に「問い合わせ方法（電話/オンライン）→オンライン時の返信目安→追加調査なら最大数日」という流れで読めますか？（Yes/No）`,
};
...
（D）②に、Leffleyさんは面接中に今週国外にいる予定だと言ったとあるが、面接の予定を変更したとは述べられていない。reschedule「～の予定を変更する」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「範囲を確かめる問題」です。まず人名の周りだけを見て、「その人について本文が言い切っていること」を判断します。**
文書の種類が「社内の連絡チャット」のときは、用件は「予定共有／研修の段取り」になりやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. 設問の形を決める：「〜について何が示されているか」は、本文に明示（またはほぼ明示）された人物情報を拾う。推測を広げない。
2. 人物のつながりを追う：欠席者の話 → 次の発言で名指しされる人物、のように同じ話題の流れで結び付ける。
3. その人の立場を抜く：「新しく入る人／採用された人／研修の対象」など、身分が分かる語を優先して拾う。
4. 選択肢を消す：本文にない行き先・行動、本文と反対の経験値、言っただけの内容を「やった」に変えるものを落とす。**ひっかかりやすい所：国外にいる＝別の場所の用事、と飛ばして考えない（本文にない移動先は捨てる）。**
5. 残ったものを確定：研修対象＝これから働き始める側、のように本文の語から言い切れるものを選ぶ。

選択肢のチェック（最後に）
✅ 選んだ内容を、本文中の「採用された側／研修対象」を示す語に結び付けて説明できますか（Yes/No）？`,
};
...
（A）①で、指導係は明日の研修会に来る予定だと言っている。currently「現在、目下」、unavailable「都合がつかない」。`,
  metacogFeedbackJa: `1) このタイプを解くには
このタイプを解くには
**これは「いちばんありそうなことを選ぶ問題」です。まず発言の前後を見て、「何に対応する提案か」を判断します。**
文書の種類を見る → よくある用件が分かる → だから候補を予想する（ただし本文で確かめる）。たとえば業務連絡のやり取りは「欠席者への対応／予定の調整」になりやすいので、まずどちらかを予想し、最後に本文で決める。

2) 解答への手順
解答への手順

1. 設問の時刻の発言だけを中心に見る場所を決め、直前の発言を「原因」、直後の発言を「評価・修正」として読む。
2. つなぎ言葉（Then など）に注目し、「何を受けての提案か」を直前の確定情報にしぼる。
3. 提案のねらいをやさしい日本語で言い換える（例：欠席する人に合わせて日程をずらす、など）。
4. 選択肢は「誰の予定の話か」「週の情報が合うか」で照合して残す。**ひっかかりやすい所：不在なのが“指導係”だと取り違える／週の情報を落として本文にない理由（資料不足・休暇）を選ぶ → 不在なのは誰か、いつ来る予定かを本文で確認して捨てる。**
5. 直後の返事が「不要・別案」になっているかで検算し、提案の向き（欠席者対応）と合うか確かめる。

3) 選択肢のチェック（最後に）
選択肢のチェック（最後に）
✅ その提案は「指導係の都合」ではなく「欠席する人の都合」を解決する内容になっていますか？（Yes/No）`,
};
...
（D）報告書の修正については話題に上っていない。revise「～を修正する」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「いちばんありそうなことを選ぶ問題」です。まず該当発言の直後を見て、「update＝何が変わったのか」を判断します。**
チャットは予定変更（期限・会議・提出物）が多い。だから最初に「期限変更／予定変更」を予想し、最後に本文で決める。

解答への手順

1. 設問の合図を確認する：*most likely mean* は、言った人の「意図」を直後の説明から読み取る。
2. 見る場所を決める：該当発言の**直後1〜2文**を最優先で読む（範囲を広げすぎない）。
3. 直後の内容を1つにまとめる：*today / due date* など**期限を示す語**を拾い、「何がどう変わったか」に言い換える。
4. 直前の前提と比べる：直前の相手の確認（*tomorrow* など）と突き合わせ、差分が「更新」の中身だと確定する。
5. 選択肢は“差分の中心”だけ残す。**ひっかかりやすい所：**最初の話題（リンク不具合）に引っ張られて「リンクが変わった」と早とちりする。→ update の直後にリンクの話がなければ捨てる。

選択肢のチェック（最後に）
✅ 選んだ内容を、該当発言の直後の *today / due date* の表現でそのまま言い換えられますか？（Yes/No）`,
};
...
（D）①で、アップロードができないというトラブルへの言及があるのみ。technical「技術上の」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「指すものを選ぶ問題」です。まず人名が出る発言を全部見て、「それ（そのメール／依頼）が何を指すか」を判断します。**
文書の種類が「オンラインチャット」のときは、発言が短くて「それ／その〜」でつながる。だから最初に“同じ出来事”を探し、最後に誰の立場かで決める。

解答への手順

1. 設問の形を見て、「本文にズバリ書いてない→発言どうしのつながりで人物を当てる」と決める（いちばんありそうなこと＋指すもの）。
2. 人名が出る発言を全部拾い、「そのメール」「依頼した」など“同じ出来事”の手がかりに線を引く。
3. 「そのメール」が何のメールかを、別の発言の「上司が期日変更をメールした」と照らし合わせて、同じメールだと決める（文書を見比べる）。
4. 同じメールの送り手＝人名、そしてその送り手は「上司」と書かれている→人名＝上司、とまとめる。
5. ひっかかりやすい所：話題が建設だからといって「建設の仕事の人」に飛びつく／最初の不具合だけで「技術サポート」と決め打ちする。→本文で“同じメール・同じ依頼”で結べないものは捨てる。

選択肢のチェック（最後に）
✅ 人名と「上司」が“同じメールを送った”という同じ出来事で、本文内でつながっている（Yes/No）？`,
};
...
（D）④で、資材の見積もりが未完了だと述べているだけ。be unsure about〜「～について確信がない」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「範囲を確かめる問題」です。まず設問の人物を見て、「その人が心配している中身」だけを判断します。**
文書の種類が「オンラインチャット」のときは、用件は「未完了の作業／予定の変更」になりやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. 設問で「誰の何か」を見る場所を決める：聞かれているのは“その人物が心配していること”。
2. チャット内でその人物の発言だけを拾う（他の人の発言は一度捨てる）。
3. その発言の中で、心配・未完了を示す言い方（still / need to / haven’t / can’t など）に印を付ける。
4. 印を付けた部分を短い言い方にまとめ、選択肢と照合する（例：見積もりがまだ終わっていない）。
   - ひっかかりやすい所：**「見ていない」→「消した」と決めつける／「会議がある」→「会議に出られない」と取り違える**（書いていない推測は捨てる）。

選択肢のチェック（最後に）
✅ 根拠は「その人物の発言」から直接言える内容だけになっている（他の人の発言や推測を混ぜていない）？ Yes / No`,
};
...
（D）Dengさんは⑤で、早めに念を押さなかったことを謝っているが、TillmanさんがDengさんに確認や念押しを目的としたリマインドメールを送るとは述べていない。reminder「思い出させるもの」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「指すものを選ぶ問題」です。まず下線部の前後を見て、「it が何を指すか」を判断します。**
文書の種類が「短い会話（連絡）」のときは、用件は「お願い／変更連絡」になりやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. 下線部の it を見たら、「代名詞＝直前の具体的な作業」を疑い、直前2〜3発言（9:14〜9:17）だけに範囲をしぼる。
2. その範囲で「未完了のタスク（これからやる必要があること）」が出た場所を探し、1つに決める（9:14の“変更が必要”の話）。
3. 謝罪→受け入れ→引き受け（大丈夫→私がやる）の流れになっているかを確認し、「今ここで引き受けた作業」だと判断する。
4. 選択肢を「本文に出た作業／本文にない作業」に分け、本文に出た作業だけ残す。**ひっかかりやすい所：**最初の「戸締まり」に引っ張られて施錠確認を選ぶ／本文にない作業（サイト投稿・メール送信）を“それっぽい”で結びつける → どちらも話題外として捨てる。

選択肢のチェック（最後に）
✅ 「it＝何？」を本文中の“変更が必要だ”と言っている箇所で指差し確認できるか？（Yes/No）`,
};
...
（D）⑦より、TillmanさんがDengさんの頼み事を聞いたことが分かる。それに対してDengさんはお返しをしたいと発言しているのみ。do～a favor「〜の願いを聞く、～に便宜を図る」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「範囲を確かめる問題」です。まず設問の主語（誰について？）を見て、「その人の習慣・担当」に当たる情報だけを拾います。**
文書の種類が「短い連絡のやりとり」のときは、用件は「お願い／お礼」になりやすい。だから最初にどちらかを予想し、最後に本文で決める。

解答への手順

1. 設問の主語を確認し、「その人について分かること」だけを探す（相手側の情報は混ぜない）。
2. 会話の終わりの「お礼・埋め合わせ・引き受け」の文を優先して読む（人物の習慣や担当が出やすい）。
3. 「daily＋時刻＋shift」を見つけたら、数・時間の言いかえをする：daily＝普段／いつも、時刻＝担当の時間帯（午前か午後）に直す。
4. 選択肢は、その言いかえ（普段＋午後など）と一致するものだけ残す。
5. ひっかかりやすい所：別の時刻（開館時刻など）を主語の人の担当だと結びつけない／休館など本文にない断定は捨てる。

選択肢のチェック（最後に）
✅ 「普段」を表す言葉（daily/usually/regularly など）を、選択肢の「普段〜」に正しくつなげられているか？（Yes/No）`,
};
...
（D）⑨で、資格があればできる作業としてソース作りが挙げられているのみ。server「給仕人」。`,
  metacogFeedbackJa: `このタイプを解くには
**これは「いちばんありそうなことを選ぶ問題」です。まず設問の時間（明日）を見て、「明日と書いてある行動」だけで判断します。**
文書の種類を見る → よくある用件が分かる → だから候補を予想する（ただし本文で確かめる）。オンラインチャットは「依頼→返事→決定」が出やすいので、決まった予定を拾います。

解答への手順

1. 設問の時間軸を決める：「明日」なので、本文から「明日」に触れている部分だけを先に集める。
2. 集めた内容を因果でつなぐ：開始できる人がいる → 明日の作業を頼む → 引き受ける、の形になっているかを見る。
3. 「起こる」を確定する条件をチェック：依頼や予定が、返事で確定しているものだけ残す。
4. ひっかかりやすい所：「〜できるようになる」「必要なときに」など条件付きの話を、明日の確定事項と取り違える。→ 条件付き・将来の可能性は捨てる。
5. ほかの選択肢は時間ズレで落とす：別の日の話、または「明日」と結びつかない話は除外する。

選択肢のチェック（最後に）
✅ 選んだ内容を「明日〜する」と言い換えたとき、本文に“依頼→承諾”または“開始日”がはっきり書かれている箇所を1つ示せますか（Yes/No）？`,
};
...
（D）直前のPlebaniさんの発言で、資格があればできる作業として生地作りに言及があるだけで、Berkさんがこの時点で生地を作る人を探すことを意図しているとは考えられない。`,
  metacogFeedbackJa: `1) このタイプを解くには
このタイプを解くには
**これは「いちばんありそうなことを選ぶ問題」です。まず引用文だけを訳さず、直前の依頼に対する返事かを見て、「何ができないと言っているのか」を判断します。**
文書の種類が「オンラインチャット」のときは、依頼→返事→代わりの人、の流れが多い。だから最初に「断り／制約」かを予想し、最後に本文で決める。

2) 解答への手順
解答への手順

1. 設問が「いちばんありそう」なら、引用文の直前にある依頼・提案を探す（会話の流れで何に答えているかを見る）。
2. 引用文が何の話題にかかるかを決める（この場合は「金曜にする作業」への制約かどうか）。
3. 引用文の直後の反応を確認して、意味を確定する（「忘れていた」＋「代わりにやる」なら、本人はその作業ができない流れ）。
4. 選択肢は「金曜に何ができないのか」だけに範囲をしぼって一致するものを残す。**ひっかかりやすい所：休みの話を「最近休んでない」など一般論に広げると範囲外。金曜の依頼に戻して捨てる。**

3) 選択肢のチェック（最後に）
選択肢のチェック（最後に）
✅ 引用文を「金曜の依頼」に差し戻して言い換えたとき、「だから金曜はできない」と自然につながるか？（Yes/No）`,
};
